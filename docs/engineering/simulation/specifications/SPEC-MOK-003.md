...
| 2026-08-22 | **What the observer accepts and does not act on, stated rather than left silent. One provision; no rule, no input, no default, no validation and no rendered output changes.** *Start-up inputs* gains a paragraph on the forwarding this specification already relies on: the observer recognizes its own three inputs and hands **every other argument** to `mokiterions::cli::parse`, which is how "identical names, identical parsing, identical validation, identical defaults and identical rejection behavior" holds by construction. Two consequences were true and unstated. `--trace-actions` is accepted and switches on what rule 9 already requires to be on, so it does nothing. `--events-path` is accepted, validated by the engine's parser and then **not acted on**: the observer writes no record stream, because `SPEC-MOK-006`'s writer is crate-private to the engine and the observer drives ticks one at a time rather than through `run_recording`. That is a **defect and is named as one**, tracked as GitHub issue 40 and deferred by the owner on 2026-08-22; until it is closed, the usage text says so, on the ground that an operator who passes the option and receives no file and no diagnostic is worse served by silence. The paragraph also records that the four engine inputs are described in the observer's usage text in the **engine's own words, byte for byte**, held equal by `mokiterions-tui/tests/options.rs`. | **Approved 2026-08-22 by the repository owner acting as technical owner**, put as its own question and separable from the `SPEC-MOK-001` and `REQ-MOK-018` amendments of the same date, with which it shares only the act that produced both usage texts. **The disclosure was the substance of the decision.** Two alternatives were put beside it and both declined: recording the same behavior without characterizing `--events-path` as a defect, declined because a later reader could then not tell that closing issue 40 is a fix rather than a change of mind; and removing the disclosure from this specification and from the observer's help text altogether, declined because it returns the observer to silently accepting an option and writing nothing, which is the state that produced the issue. The implementation agent had opened issue 40 on the owner's direction and had written the disclosure without having it put to it; that sentence is now the owner's. |
...
| 2026-08-29 | **The disposition the 2026-08-24 row left owed is written: *Start-up inputs* gains the five live-run options the shared parser now accepts, each recognized and refused by this program. One paragraph and six bullets; no new rule, no new input, no new default, no new validation, no new rendered output and no new exit code.** The count of forwarded engine options outside the input table is edited **three to eight**. **Five and not the three that row left owed**: `--connector-path`, `--live` and `--spend-ceiling` are `ADR-MOK-007`'s, and `--transcript-output` and `--prices` are named by that ADR nowhere — the first arriving with the recording host and the second under `SPEC-MOK-007` rule 14.3a later on 2026-08-29 — so the disposition is stated over what the shared parser accepts rather than over what an artifact foretold. The provisions: all five are refused at start-up by `options::parse`, **before** the argument reaches `mokiterions::cli::parse`, with `configuration error: `, the message quoted in the paragraph, this program's usage text and exit **2**, which is the exit `2` sentence's own case and not a new code; the message names the option and this host's limit, calls it **unknown** nowhere because the shared parser accepts it, and offers no substitute source under rule 20.3; refusing before the shared parser is what decides which of two applicable refusals the operator reads, rule 18.4.3's "only used by `--policy llm`" being true and unhelpful here; `--policy llm` with neither a transcript nor a live-mode selection stays the shared parser's refusal and is **not** one of the five, the guarantee that `llm` implies a transcript having stopped being that parser's on 2026-08-28 and being restored by the `--live` refusal rather than by a check of this program's own; and the usage text's five-option sentence and its exit-status clause are recorded as this host's own words about its own capability. **Every figure and every quoted string is measured at the candidate `7f9e20a`**, including the exit status of each of the five and of `--policy llm` alone, each of which reads **2**. **What this row does not write.** No live mode, no connector, no provider call, no credential and no socket reaches this host, and rule 20.1's split with rules 6.1 and 6.2's reason is untouched. The byte-identity set stays at **five** entries, because an option this host refuses has no entry here to hold equal to anything. `--transcript-path`'s disposition, this host's own three inputs, the `--trace-actions` and `--events-path` bullets, GitHub issue 40, `--export`'s sentence, the *Declared dependency set* and both filesystem provisions are all unmoved. | **Written 2026-08-29 by the implementation agent under `ADR-MOK-007`'s authorization**, discharging `WO-MOK-026` item 16's second clause — "the amendments `ADR-MOK-007` requires … of `SPEC-MOK-003`'s *Start-up inputs* for the observer's disposition of these three options" — and the 2026-08-24 row's own reservation, "It is `WO-MOK-026`'s to write, in this paragraph, when the engine gains the options." The authorizing act is the repository owner's **"i approve the artifact pack"** of 2026-08-23, taken as accountable technical owner, and this specification is in `WO-MOK-026`'s approved execution scope. **The agent decided none of the substance**: the disposition is `ADR-MOK-007`'s, the refusal it describes was implemented at `0f256f0` and `055e2e2` under this same work order, and every figure and quoted string above is a measurement of the candidate rather than a choice. **Two things are disclosed rather than decided.** First, the disposition covers **five** options where the authorization names three; the two beyond it are the same shape of option, they are accepted by the same parser, and the alternative — writing the disposition over three and leaving `--transcript-output` and `--prices` undescribed — would leave this section false about two options an operator can type. `ADR-MOK-007` gave no disposition for either because neither existed when it was approved. Second, the frontmatter's `updated` field reads `2026-08-24` and this row is later. Whether correcting it is a statement of fact an implementation agent may make or bookkeeping on an approved artifact it may not is a question already put to the repository owner in `SPEC-MOK-007`'s 2026-08-29 row, where the same staleness arises and where `SPEC-MOK-002`'s and `SPEC-MOK-007`'s own rows read it two different ways; it is not re-decided here and the field is left as it stands. **No row above is touched by this one** and no record bound to a commit is re-opened: the 2026-08-24 row's "only the transcript path exists in the shared parser at this candidate" is true of the tree it names, and `VREC-MOK-005`, which binds this specification, is not edited. |
...
- `--trace-actions` is accepted and has nothing to switch on. Rule 9 requires tracing to be on here, so the
  observer sets it unconditionally, and passing the option changes no input, no run and no frame.
- `--events-path` is accepted, validated by the engine's parser, and then **not acted on**. The observer writes no
  structured record stream: `SPEC-MOK-006`'s writer is crate-private to the engine, and the observer advances one
  tick at a time rather than through the whole-run path that writes records. An operator who passes it receives no
  file and no diagnostic. **This is a defect, not a design**, tracked as GitHub issue 40 and deferred by the owner
  on 2026-08-22. Until it is closed the usage text states it, because silence here misleads and a sentence does
...
**Amended 2026-08-29 under `WO-MOK-026`: the engine has the options, and the disposition the paragraph above leaves
owed is written here.** The count of engine options the forwarding carries reads **eight** — it read two on 2026-08-22
and three on 2026-08-24 — and the five that arrive share one disposition. They are more than the three that paragraph
left owed. `--connector-path`, `--live` and `--spend-ceiling` are `ADR-MOK-007`'s, the transcript path having been
written on 2026-08-24; `--transcript-output` and `--prices` are named by that ADR nowhere, the first arriving with the
recording host and the second under `SPEC-MOK-007` rule 14.3a later on 2026-08-29. What puts an option in this
paragraph is that the shared parser accepts it, not which artifact foretold it, and that is why the disposition is
stated over five rather than over three.

- **All five are recognized and refused at start-up by this program, and not by the shared parser.** That is
  `SPEC-MOK-007` rule 18.4.2 and it is the disposition `ADR-MOK-007` states. The refusal is `options::parse`'s and
  fires **before** the argument reaches `mokiterions::cli::parse`; it writes `configuration error: ` with the message
  below, follows it with this program's usage text, and exits **2**. The exit `2` sentence above is unmoved and this is
  one of its cases: an option this program cannot honour is an invalid invocation of this program. Measured at the
  candidate for each of the five, and for each the status is 2.
- **The message names the option the operator typed and this host's own limit as the reason, and offers no substitute
  source**: *"`<option>` belongs to the Mokiterions binary: this program only replays `--policy llm`, so it starts no
  connector program, asks no model and spends nothing. Record a live run with that binary, then watch it back here with
  `--transcript-path <path>`."* It does not call the option **unknown**, because the shared parser accepts it and
  calling it unknown would be false. It is not silently ignored, which is rule 18.4.1's whole subject and the shape
  GitHub issue 40 tracks. And it names no other source, for rule 20.3's reason: a run under another label is not this
  run.
- **Refusing before the shared parser is what decides which of two applicable refusals the operator reads.** Rule
  18.4.3 would answer `mokiterions-tui --spend-ceiling 2` with "only used by `--policy llm`", which is true and which
  sends the operator to select a source that leaves this program refusing the same option for a second reason. What is
  wrong with the invocation is the program and not the policy, so this host says so and never examines the value.
- **`--policy llm` with neither a transcript nor a live-mode selection is the shared parser's refusal and is not one of
  the five.** It was this specification's own concern for one day: rule 13.2's check accepts `llm` with a transcript
  **or** a live-mode selection, so the guarantee that `llm` implies a transcript stopped being the parser's on
  2026-08-28. What restores it here is the `--live` refusal above rather than a check of this program's own —
  `--policy llm --live` is refused on `--live`, and `--policy llm` alone is still refused in the engine's own words with
  exit 2, both measured at the candidate.
- **This host's own three inputs, `--transcript-path`, and the `--trace-actions` and `--events-path` bullets above are
  all untouched.** GitHub issue 40 is neither closed nor worsened, and `--export`'s sentence about a value validated as
  a string is unaffected: none of the five is ever opened, read, interpreted or retained, because the refusal precedes
  any examination of the value.
- **The usage text says it, in this host's own words.** *"So the five options that make a recording rather than read
  one belong to the Mokiterions binary alone: `--connector-path`, `--live`, `--transcript-output`, `--spend-ceiling` and
  `--prices`. That binary accepts them and this one refuses them, saying which, rather than accepting one and doing
  nothing with it."* This is a host-capability statement and not a shared input's meaning, which is the division the
  section's closing paragraph draws; the exit-status sentence there gains "or belongs to the Mokiterions binary alone"
  among exit `2`'s causes. The five have **no entry of their own** in this program's synopsis, and that is asserted
  rather than left to hold: `mokiterions-tui/tests/options.rs` holds the refused list equal to the set the engine's
  parser accepts, and requires an entry in the engine's help and none in this program's.

**What this amendment does not write.** The observer gains no live mode, no connector, no provider call, no credential
and no socket, and rule 20.1's split and the reason rules 6.1 and 6.2 give are untouched. It adds no input to the table
above, no default, no validation and no rendered output: a refusal is the absence of all four. It states nothing about
the engine's disposition of these five options, which is `SPEC-MOK-001`'s and `SPEC-MOK-007`'s. And it does not make the
five shared inputs whose meaning this specification may restate — the byte-identity set stays at the five entries the
paragraph below names, because an option this host refuses has no entry here to hold equal to anything.
...
truncation marker when `truncated` is set. Where the row's width will not hold every field, fields are shed in the
fixed order of clause 4. A field is present with its whole value or it is absent: no field is abbreviated, rounded,
re-based or cut.
...
2. The candidate commit is displayed when it was supplied to the build as a compile-time value, and the field is
   absent otherwise. The observer does not read repository files, invoke git, or guess. **Supplied means non-empty.**
   A compile-time variable that is set to the empty string is not a supplied commit and the field is absent, because a
   field marker carrying no value presents neither a commit nor the absence of one.
3. The footer contains no wall-clock time, no absolute path, no environment variable and no credential.
4. **Order of loss.** The row presents every field the width holds. Where it does not hold them all, fields are shed
   one at a time, in this order, until the row fits:

   | Shed | Field |
   |---|---|
   | 1st | the candidate commit of clause 2 |
   | 2nd | the retained-event count, with its truncation marker |
   | 3rd | the current tick |
   | 4th | the active decision source |
   | 5th | the resource density as supplied |
   | 6th | the configured tick limit |

   The entropy seed is never shed. No field is shed while the row fits, and none is shed out of this order. The
   candidate commit is shed before every field of the preamble, so stamping a build never costs the row a field this
   rule requires.
5. **The count and its marker are one field.** The truncation marker the preamble requires when `truncated` is set is
   part of the retained-event count and is present wherever that count is present. The two are shed together and never
   separately, so no row states a retained count that a reader could take for the whole run.
6. **What the floor guarantees.** At the floor of `34 × 22` the entropy seed is always present: a `u64` seed is at most
   20 digits and the row is 34 columns. Nothing beyond the seed is guaranteed there. `SPEC-MOK-001`'s start-up contract
   accepts a seed and a tick limit anywhere in `u64`, and a 20-digit seed beside a 20-digit tick limit occupies 43 of
   those 34 columns with the other four fields already shed. That is arithmetic and no rendering choice reaches it;
   clause 7 fixes what the observer does about it.
7. **A shed field is not a clipped one.** The row is never longer than the pane and is never cut at the pane's edge.
   `REQ-MOK-024` forbids content clipped in a way that could be read as a different value, so a tick limit of
   `18446744073709551615` presented as `18446744073` is a defect at every width and not a narrow-viewport degradation.
   Where the width holds fewer fields the row carries fewer fields, and a field the row cannot hold whole is absent.
8. **Labelling is the implementation's, values are not.** How each field is labelled, and how compactly, is
   unspecified: `seed 42` and `s42` both discharge this rule. Every value is presented in full and in the radix in
   which it was supplied, so an abbreviated label never becomes an abbreviated value.
