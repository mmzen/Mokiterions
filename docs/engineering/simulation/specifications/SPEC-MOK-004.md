...
| 2026-08-29 | **Rule 11's figures re-measured at `WO-MOK-026`'s candidate `2574ff9`, six tests arriving and none departing or being renamed; no target count, file count or binary count moves; rule 6 re-measured over the difference between the two candidates and found unmoved; and two edits made to this file at `66d116d` put on the record. Rule 1 does not move.** Rule 11 gains a paragraph, an arrivals table and two further paragraphs rather than an edit, the way the row above wrote its share. **Its figures at this candidate**: the observer's total **202**, unmoved, and the engine's **282**, so the workspace's **484**, of which **481** execute and **3** are the same `#[ignore]`d instruments; the engine's 282 is `SPEC-MOK-002` rule 7's internal **175** and rule 8's public **107**. **No figure above is superseded**: the row above's 478, 475, 276 and 102 are true of `7f9e20a` and are kept there. **The six arrivals are reconciled by qualified name and not by subtraction** — one in `mokiterions-core/src/simulation.rs`, for rule 14.2a's arithmetic, and five in `mokiterions-core/tests/replay.rs`, for the two replay identities, the populated-transcript case, the no-fallback case and the credential guard — and the arrivals table names what each asserts. Both files already declared tests, so **the engine's eleven public-tier targets, the observer's nine, the observer's four internal-tier files and the twenty-five test binaries all stand**, and the counts are produced twice: `cargo test --workspace --locked` reports **481 passed, 0 failed, 3 ignored**, and a per-file `#[test]` count agrees target by target at **484**. **Rule 6's interface is unmoved** at **95** items, **26** public fields and **121** `pub` lines, and here that is established over the difference rather than by re-enumerating the modules: exactly two files under either package's `src/` change between `7f9e20a` and this candidate — `mokiterions-core/src/simulation.rs` and a three-line comment in `mokiterions-tui/src/options.rs` — and **no line carrying `pub` is added or removed in either**, so there is no line on which a member could have arrived or departed. The four evidence files this candidate retains declare no test and move no figure this specification states. **One assertion is generalized, and rule 12 is why that is said out loud**: the credential guard applies rule 11.6's ten patterns to six retained captures where it named two, every pattern it asserted of those two is still asserted of them unchanged in predicate and in strictness, nothing is weakened, split, merged, renamed or relocated, and only its failure messages become path-bearing. **Rule 6's growth stays OUTSTANDING** exactly as the 2026-08-24 row states it, neither moved nor discharged, and that row's `#[ignore]` amendment is untouched; the third `[[bin]]` target's contradiction of `SPEC-MOK-002` rule 1 likewise stays a defect recorded for the owner with none of its three measured repair routes chosen. **Two edits `66d116d` made to this file are recorded here, because that commit judged the record needed no row and one of them falsified the row above.** The frontmatter's `updated` field now reads **2026-08-29**: the row above says it reads `2026-08-24` and is left as it stands, which was true of the tree that row was written on, and this row states the later fact instead of editing that cell. And rule 11's sentence calling the observer's internal-tier count unmoved at three is corrected to **four**, named as a file count with the four files listed inline beside the figure they were drawn from — that sentence is this branch's own, absent at merge base `d96cced` and arrived at `eeff19a`, and the count is four at both revisions, so the claim of unmovedness was true when written and only its number was wrong. **No row above is touched by this one**, and `VREC-MOK-023` and `VREC-MOK-024` bind earlier content of this specification at `f633eda` and `b0c18b8` and are not re-opened. The implementation agent measured every figure, wrote this text and decided none of the substance. | **Covered by `ADR-MOK-007`, approved 2026-08-23 by the repository owner acting as accountable technical owner with the words "i approve the artifact pack"**, on the same clause as the row above: its *Required amendments* section states that "**Rule 11's test-count figures** move for every stage that adds a test, by that rule's own delegating clause", and its *Status* clause gives this work order the share this row writes. This row completes `WO-MOK-026` item 16's third clause for the tests that arrived after `7f9e20a`, which the row above could not state because they did not yet exist, and that item's closing instruction "**`SPEC-MOK-004` rule 1 does not move**" is obeyed here as well. **Nothing in the figures is a decision**: the test counts are `cargo test`'s own output at the candidate, reproduced by an independent count of the attribute, and rule 6's three figures rest on a check no judgement enters — whether a line carrying `pub` was added or removed in the two source files that changed. **One thing does need the owner's eye, and it is named rather than left in a commit message.** Moving the `updated` field was the implementation agent's act at `66d116d`, taken on the ground `SPEC-MOK-007`'s 2026-08-29 rule 11.7 row states for that file's identical field — this branch added the later rows and omitted the field, so the move completes this branch's own edit rather than correcting an inherited fact — and that row, field move included in its text, was approved by the owner. The row above had put the competing reading, that it is bookkeeping on an approved artifact an implementation agent may not do, and this file now reads it one way. **If the owner reads it the other way the repair is one field**, and no figure, no rule and no record in this file depends on which way it goes. |
...
**Re-measured for `WO-MOK-026` at its candidate `2574ff9`**, the observer's total is **202**, unmoved, and the engine's is
**282**, so the workspace's is **484** — of which **481** execute, the three instruments being the same three. The
engine's 282 is `SPEC-MOK-002` rule 7's internal **175** and rule 8's public **107**. **The paragraph above is not
contradicted**: its 478, 475, 276 and 102 are true of `7f9e20a` and are kept on the record. **Six tests arrive between the
two candidates, none departs and none is renamed**, and all six are the engine's, reconciled by qualified name rather than
by count:

| Where | Tests | What arrives |
|---|---|---|
| `mokiterions-core/src/simulation.rs` | 1 | the reasoning share of the output count billed once rather than twice, priced at a rate distinct from the output rate so that a wrong split cannot hide, asserted at the equal rate as well because the double bill is the whole of the reasoning count's cost, a reported zero and an absent count both held harmless, and the clamp exercised where a reported reasoning count exceeds its output count — `SPEC-MOK-007` rule 14.2a, under `SPEC-MOK-002` rules 7 and 8 rather than this rule |
| `mokiterions-core/tests/replay.rs` | 5 | the live transcript replaying into the run its record stream describes and, the run's own output now being retained, into that output byte for byte; the same for the rejected first attempt's transcript against its own retained output; the live transcript populated where the recorded one is absent; no fallback and no retried attempt recorded in it; and rule 11.6's ten credential shapes refused across all six retained captures, with the provider endpoint accounted for on the connector's readiness line alone |

**No target count, no file count and no binary count moves.** The engine's public-tier target count stays at **eleven** and
the observer's at **nine**, its internal-tier file count at **four**, and the test-binary count at **twenty-five**: the six
arrivals land in two files that already declared tests, and no target of any kind is added. The figures are measured by
`cargo test --workspace --locked`, which reports **481 passed, 0 failed, 3 ignored**, and independently by counting
`#[test]` per file, which agrees target by target and yields **484** for the reason the paragraphs above give, that the
count reads the attribute and not the run. **Rule 6's interface is unmoved** at the **95** items, **26** public fields and
**121** `pub` lines, and here that is measured over the difference rather than module by module: exactly two files under
either package's `src/` change between `7f9e20a` and this candidate — `mokiterions-core/src/simulation.rs`, for rule
14.2a's arithmetic and its one new case, and a three-line comment in `mokiterions-tui/src/options.rs` — and **no line
carrying `pub` is added or removed in either**, so there is no line on which a member could have arrived or departed. The
same candidate retains four evidence files, which declare no test and move no figure this rule states.

**One assertion was generalized and rule 12 is why that is said out loud.** The credential guard now applies rule 11.6's
ten patterns by iterating a list of six retained captures where it named two. Every pattern it asserted of the two files it
covered is still asserted of them, unchanged in predicate and in strictness, and four files are added; only the failure
messages become path-bearing rather than naming one file in prose. Nothing is weakened, split, merged or renamed, which is
what rule 12 forbids, and the assertions are not relocated at all.
