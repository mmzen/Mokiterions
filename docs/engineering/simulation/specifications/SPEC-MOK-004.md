...
As corrected for `WO-MOK-012` and measured on the record-stream candidate tree, the observer's total is **127**
...
the one `WO-MOK-011` retained, which is recorded in `WO-MOK-012`'s `interface.txt`. The engine's own interface grows by
