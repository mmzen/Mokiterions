...
`tests/options.rs` reads 8 rather than 7 and `tests/render.rs` reads 10 rather than 8 as corrected for `WO-MOK-010`,
which `VER-MOK-010` obliges to assert `REQ-MOK-032` at the frame. Three tests arrive and none departs:
...
places them in the public tier and nothing widens. The internal tier of rule 10 is unchanged at 32: `WO-MOK-010`
...
As corrected for `WO-MOK-010`, the observer's total is **115** — the three of rule 9 above — the engine's is **78**,
...
No test was lost: the 21 additions and 0 removals are reconciled name by name in `WO-MOK-010`'s `test-census.txt`.
