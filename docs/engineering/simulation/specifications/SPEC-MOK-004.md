...
| 2026-08-20 | **Recorded test-count figures corrected for `WO-MOK-018`, which adds thirty-four tests for `REQ-MOK-042` through `REQ-MOK-046`.** Rule 11 states the obligation this row discharges: "a work order that adds a test corrects these figures here, and one that loses a test has a defect". **Rule 11**: the workspace's executed total rises from 212 to **246**; the engine's from 85 to **119**, split 68 internal and 51 public because 246 is otherwise not reproducible; the observer's stays at **127**, because this work order adds a stream the observer does not read and no observer test is added, removed or moved. The thirty-four arrivals are attributed to the three targets that run them — fourteen in `mokiterions-core/src/simulation.rs`, three in `mokiterions-core/tests/cli.rs`, and seventeen in `mokiterions-core/tests/records.rs` — and none departs, which is measured by qualified name rather than by count: 34 additions and **0 removals** against the baseline log, so no rename can hide in the diff. `mokiterions-core/tests/records.rs` is recorded as a new engine public-tier target, admitted by `SPEC-MOK-002` rule 8's closing sentence in the same way `tests/naming.rs` and `tests/decisions.rs` were, without obliging that rule's initial-arrangement table. **Rules 9 and 10 are unchanged**, both being observer rules. **Rule 6 is unchanged at 94 items, 118 `pub` lines and 24 public fields**, measured at five revisions rather than assumed: the mechanical enumeration of `mokiterions-tui/src` is byte-identical to the one `WO-MOK-011` retained, and the mechanical figure reconciles with the recorded one by the same documented subtraction of `lib.rs`'s seven `pub mod` declarations. The engine's own interface grows by one parameter on `execute` and by no item; `SPEC-MOK-002` rules 4 and 5 as amended the same day are the authority for that, not this rule. Nothing else changes — no target name, path, package name, tier boundary, hook or prohibition, and no item's visibility widens. Every figure is measured on one `cargo test` invocation at the workspace root against the candidate tree, with every source touched first so that nothing reported a cached result. | **Approved 2026-08-20 by the repository owner acting as technical owner**, in a separate act after the candidate commit `50364a3` and after this cell was written. The confirmation the cell below asks for is the confirmation given: that `SPEC-MOK-004` belongs in this chain, and that no figure in this row is wrong — the workspace's executed total at **246**, the engine's at **119** split 68 internal and 51 public, the observer's unchanged at **127**, rules 9 and 10 unchanged as observer rules, and rule 6 unchanged at 94 items, 118 `pub` lines and 24 public fields. **The text below is what this cell said at the candidate commit and is kept unedited**, not because a superseded statement is worth preserving for its own sake, but because `evidence/WO-MOK-018/amendment-approvals.md` measures this cell's wording and that measurement was taken before this approval existed; editing the cell into agreement with a later decision is the thing this repository's evidence rule forbids. Read the two together: the row was unapproved when the packet measured it, and it is approved now. **The substance requires no owner decision and is not claimed as approved. Recorded rather than claimed: `SPEC-MOK-004` was not among the three amendments — `ARCH-MOK-001`, `SPEC-MOK-001` and `SPEC-MOK-002` — that the repository owner approved on 2026-08-20 with this chain, and `ADR-MOK-005`'s *Required amendments* section does not name it.** This row is written as the discharge of rule 11's own delegating clause, which obliges the correction of these figures by the work order that causes them and makes the figures measurements rather than decisions. What remains for the repository owner acting as technical owner is the confirmation that `SPEC-MOK-004` belongs in this chain at all and that no figure in this row is wrong; that confirmation is **OUTSTANDING** and is recorded as such in `evidence/WO-MOK-018/amendment-approvals.md`, alongside the eight manual assessments `VER-MOK-012` requires. The implementation agent measured, wrote this text and decided none of it. **No row above is touched by this one**, and no record bound to a commit is re-opened. The measurements are retained in `evidence/WO-MOK-018/gates.txt`, `evidence/WO-MOK-018/analysis/census-reconciliation.txt` and `evidence/WO-MOK-018/interface.txt`. |
| 2026-08-21 | **Recorded figures re-measured on the merge of `master` at `fa065cc` into this branch, and corrected for `master`'s `WO-MOK-016`, which adds thirty-eight tests and one private item and corrects neither figure — the first measured and recorded as owed, the second not reported at all.** Two rows above are the precedent for the two halves of this one: the 2026-08-19 `WO-MOK-010`/`WO-MOK-007` row for correcting another branch's uncorrected figures in the first tree in which both sides' additions exist, and the 2026-08-19 `WO-MOK-011` row for re-measuring this branch's own figures after a merge and keeping the superseded ones on the record. **Rule 9**: `tests/verification.rs` rises from 20 to **21** and the public tier from 100 to **101**, the one arrival being `the_social_source_is_rejected_only_as_the_specification_admits`. The file rises by one and not by two because `no_shipped_decision_source_has_a_proposal_rejected` is **renamed** to `no_source_confined_to_the_valid_action_list_has_a_proposal_rejected` and a rename is not an arrival, which is the accounting `tests/layout.rs` already received in the `WO-MOK-013` row; the rename is the narrowing a fourth decision source forces, its assertions verbatim over the three sources it still sweeps, and the arrival is what covers the fourth. The file's subject line gains **the admitted rejections** for it. **Rule 10**: no test count moves — the four rows stay at 20, 9, 4 and 8, the total at **41**, and the hook figure at 7 of 20, because every one of `master`'s fifteen observer arrivals is rule 9's or already counted there — and `src/render.rs`'s private-item count rises from 48 to **49**, **31** functions and 18 constants, its one arrival the private `action_text`. **Rule 11**: the observer's executed total rises from 127 to **142**, the engine's from 119 to **156**, and the workspace's from 246 to **298**, with the fifty-two arrivals attributed to the ten targets that run them, the engine's 96-internal and 60-public split recorded because 298 is otherwise not reproducible, and the observer's 142 cross-checked as rule 10's 41 plus rule 9's 101. **Nothing is removed in either direction**, measured by qualified name rather than by count: the merged set holds all 264 of `master`'s names and all but the two renamed of this branch's 246, so the union of the two sides is 300, their intersection 210, and the merge 298 — which is also why the arrivals counted as arrivals are 52 and not the 54 names the raw set difference shows. **The 226 rule 11 recorded for the workspace was 38 low from the moment `WO-MOK-016` landed on `master`**, and the 48 of rule 10 one low with it. **The two stand differently and are not put here as one thing.** Rule 11's was measured by that work order and recorded as owed: its `post/test-census-reconciliation.md` §9.4 applies this rule's own classification, reaches the observer's **142**, the engine's **122** and the workspace's **264** with the engine split 82 internal and 40 public, and validates the classification by reproducing all seven of the `WO-MOK-013` row's figures at the commit those figures were written for; `amendment-approvals.md` §5 then records the amendment as one "this work order owes and has not made", `completion-summary.md` §17.4 carries it as finding 1, and §9.4 closes by referring the timing to the owner — "whether that re-derivation happens under this work order or the next is an owner's call". This row is where that referral is answered, and the figures it states are the same measurement re-taken on the merged tree rather than a fresh accusation; `VREC-MOK-016` records 264 passed and 264 unique test names at `4539601`, and `analysis/updated-tests.py`'s `"totals": (226, 264)` and `completion-summary.md` §13.1's four-column census carry the same step. **That record's own status is stated precisely, because an earlier draft of this row called it verified and it is not**: at `4539601` it is a `ready` candidate whose populated `verified_at` is a capture timestamp, and `master` at `aeca808` has since transitioned it to `superseded` — 2026-08-21T12:57:39Z, on the assurance owner's authority, `superseded_by = ["VREC-MOK-017"]`, that successor verifying `WO-MOK-016` at `ecba9fe`. **The 264 does not depend on any of that**: it is a `cargo test` count that the work order's own census reproduces independently of the record's status, which is why this row's figures do not move and only the citation is corrected. Rule 10's is not on that record at all: no file of that packet connects the private `action_text` its change-surface table names to this rule's item count, and its finding 1 names rule 11 only, so **48 to 49 is a figure the owner is shown here for the first time** and is disclosed as such rather than as a correction already reported. Neither is absorbed into this work order's arrivals, on the `WO-MOK-013` row's precedent for a figure that "was itself one high", and both are corrected here because this is the first tree in which both sides exist — Phase 4a's 34 arrivals and `master`'s 38 are in one tree, and a row correcting one alone would leave this rule stating a number no tree runs. **Rule 6 is unchanged at 94 items, 118 `pub` lines and 24 public fields**, measured rather than assumed: the observer's enumeration on the merged tree is byte-identical to `master`'s, and its one difference from this branch's is a line of `options::USAGE`'s value naming the fourth policy, which adds no item and widens none. The engine's interface reads 49 items and 43 public fields on all three trees under `SPEC-MOK-002` rules 4 and 5 rather than this rule. Nothing else changes — no target, target name, path, package name, tier boundary, hook or prohibition, and no item's visibility widens. Every figure is measured on one `cargo test` invocation at the workspace root against the merge commit `1e09f85`, and the three censuses, the enumerations and the run log are retained in `evidence/WO-MOK-018/merge/`. | **OUTSTANDING.** Drafted by the implementation agent for ratification by the repository owner acting as technical owner, under the correction procedure the owner set on 2026-08-21: the agent drafts each correction and the owner ratifies each. **Every figure in this row is a measured outcome rather than a decision** — the test counts are one `cargo test --workspace` invocation's per-target output at `1e09f85`, the three censuses are each target's own `--list` output reconciled by qualified name, and the item and interface counts are enumerations of the same tree — and **no provision of this specification is added, removed or reworded**. What is put for ratification is therefore not the numbers but three statements about other work. First, that this row is the answer to a referral `WO-MOK-016` made rather than a finding against it: that work order measured rule 11's figures, recorded the amendment as owed and unmade in `amendment-approvals.md` §5, and left the choice of which work order re-derives them to the owner — so ratifying this row is the act that decides it happens under `WO-MOK-018`. Second, that rule 10's 48 is a defect nothing on the record has reported before this row, which is a weaker position for the owner to be shown a figure in and is stated as such. Third, that this branch's merge is the right place for both rather than `master`, this being the first tree in which both sides' additions exist. **No row above is touched by this one**, and no record bound to a commit is re-opened: `VREC-MOK-012` measured 246 at `50364a3` and was correct there, `VREC-MOK-016` measured 264 at `4539601` and was correct there, and the merge's figures are carried by a new verification record at the merge commit rather than by editing either — which is the repository owner's decision of 2026-08-21, taken in the same act that set this procedure. The implementation agent measured, wrote this text and decided none of it. |
...
| `tests/verification.rs` | the cross-cutting properties: non-perturbation, export fidelity, presented-value fidelity, the authority verdict, colour independence, the admitted rejections | 21 |
| **Total** | | **101** |
...
`tests/verification.rs` reads **21** rather than 20 and the total **101** rather than 100, as corrected on the merge of
`master` at `fa065cc`. One test arrives in this tier and one is renamed, both under `master`'s `WO-MOK-016`:
`the_social_source_is_rejected_only_as_the_specification_admits` arrives, and
`no_shipped_decision_source_has_a_proposal_rejected` becomes
`no_source_confined_to_the_valid_action_list_has_a_proposal_rejected`. **That is why the file rises by one and not by
two**, and it is the accounting `tests/layout.rs` already received above: under rule 12 a rename is not a departure and
its new name is not an arrival. The rename is the narrowing a fourth decision source forces. `SocialDecisionSource`
proposes targeted actions, which `SPEC-MOK-001` rule 3 keeps off the valid-action list on purpose, so the old name's
claim about every shipped source stopped being true of all four while the assertion itself stayed verbatim over the
three it sweeps. The arrival is what covers the fourth: a rejection under `social` is specified behavior rather than a
fault, so what it asserts is that the observer presents one of the engine's own grounds for it and invents none.
**The file's subject line gains "the admitted rejections"** for it. `master` reached 101 in this tier without correcting
this rule; `WO-MOK-016` measured that and recorded the correction as owed, and the 2026-08-21 row of the amendment
record is where it is made.
...
|---|---|---|
| `mokiterions-tui/src/render.rs` | 20 | assert drawing internals; the module declares 49 private items — 31 functions and 18 constants — against 2 public ones, and 7 of the 20 additionally reach a hook |
...
The same row's private-item count reads **49** rather than 48 — **31** functions and 18 constants — as corrected on
the merge of `master` at `fa065cc`. Its one arrival is `action_text`, added by `WO-MOK-016` and private to the module.
**No test of this rule arrives or departs with it**: the four rows stay at 20, 9, 4 and 8, the total at **41**, and the
hook figure at 7 of 20, because `master`'s fifteen observer arrivals are all rule 9's or already counted above.
`master` reached 49 without correcting this rule, and unlike rule 11's figure this one is on no
record before the 2026-08-21 row of the amendment record: that work order's change-surface table names the function and
nothing in its packet reaches this count, so the row states it as a figure first reported there. Rule 6's three figures are
untouched by it, measured rather than assumed: `action_text` is private, and the observer's whole enumeration on the
merged tree is byte-identical to `master`'s at **94** items, **118** `pub` lines and **24** public fields.
...
As corrected for `WO-MOK-018` and measured on the record-stream candidate tree, the observer's total is **127**
...
the one `WO-MOK-011` retained, which is recorded in `WO-MOK-018`'s `interface.txt`. The engine's own interface grows by
one parameter on `execute` and by no item, which `SPEC-MOK-002` rules 4 and 5 as amended are the authority for.

As re-measured on the merge of `master` at `fa065cc` into this branch — commit `1e09f85`, the first tree in which this
work order's record stream and `master`'s `WO-MOK-013` and `WO-MOK-016` all exist — the observer's total is **142** and
the engine's is **156**, so the workspace's is **298**. The merge itself adds no test and loses none. The fifty-two
arrivals it brings from `master`, each measured from the target that runs it:

| Where | Tests | What arrives |
|---|---|---|
| `mokiterions-core/src/simulation.rs` | 28 | `WO-MOK-016`'s decision and resolution layers, in the engine's internal tier under `SPEC-MOK-002` rule 7 |
| `mokiterions-tui/tests/render.rs` | 10 | `WO-MOK-013`, rule 9 above |
| `mokiterions-core/tests/viability.rs` | 3 | `WO-MOK-016`, in the engine's public tier under `SPEC-MOK-002` rule 8 |
| `mokiterions-core/tests/cli.rs` | 2 | the same |
| `mokiterions-core/tests/decisions.rs` | 2 | the same |
| `mokiterions-tui/src/render.rs` | 2 | `WO-MOK-013`, rule 10 above |
| `mokiterions-tui/tests/verification.rs` | 2 | one `WO-MOK-013` and one `WO-MOK-016`, rule 9 above |
| `mokiterions-core/tests/process.rs` | 1 | `WO-MOK-016`'s fourth source at the process boundary |
| `mokiterions-core/tests/termination.rs` | 1 | `WO-MOK-016`, in the engine's public tier |
| `mokiterions-tui/tests/layout.rs` | 1 | `WO-MOK-013`, rule 9 above |

Thirty-seven of the fifty-two are the engine's and fifteen the observer's, which is what carries 119 to 156 and 127 to
142. The census is reconciled three ways rather than two, by qualified name rather than by count: `master` at `fa065cc`
runs **264**, this branch at `fa0bfd9` runs **246**, the merge at `1e09f85` runs **298**, and 298 is 264 plus this work
order's 34 and 246 plus `master`'s 52. **Nothing is removed in either direction.** The merged set holds every one of
`master`'s 264 names, and the only two of this branch's 246 it does not hold are the two `master` renamed —
`the_log_is_ten_rows_only_where_both_thresholds_are_met` under `WO-MOK-013` and
`no_shipped_decision_source_has_a_proposal_rejected` under `WO-MOK-016`. That is why the two sides' union is 300 while
the merge is 298, and why the arrivals counted as arrivals are 52 rather than the 54 names the raw set difference
shows. The two sides' intersection is 210.

**The 226 the `WO-MOK-013` paragraph above records for the workspace was 38 low from the moment `WO-MOK-016` landed on
`master`**, and this is where that is corrected, because this is the first tree in which both sides exist — the same
reading the 2026-08-19 correction of `master`'s `WO-MOK-007` took. It is a conformance defect in that work order and
not in this specification, and it was measured rather than missed: `VREC-MOK-016` records **264 passed and 264 unique
test names** at `4539601`, and `WO-MOK-016`'s own evidence carries the step as
`analysis/updated-tests.py`'s `"totals": (226, 264)` and as `completion-summary.md` §13.1's four-column census. Rule
10's item figure went the same way, at 48 recorded against 49 declared. Neither record is re-opened for it and neither
figure is absorbed into this work order's own arrivals.

The engine's split is **96** internal and **60** public, and the observer's is rule 10's **41** and rule 9's **101**.
The engine's holds under `SPEC-MOK-002` rules 7 and 8 rather than this rule, and both are recorded here because 298 is
otherwise not reproducible: 96 + 60 + 41 + 101 = 298. No target is added or removed by the merge — every arrival lands
in a file this rule, rule 9, rule 10 or `SPEC-MOK-002` rule 8 already names, and the observer's target count is
unchanged. Rule 6's interface is unchanged at **94** items, **118** `pub` lines and **24** public fields, measured on
the merged tree rather than assumed: the enumeration of `mokiterions-tui/src` is **byte-identical to `master`'s**, and
its one difference from this branch's is a line of `options::USAGE`'s value naming the fourth policy, which adds no
item and widens none. The engine's own interface reads 49 items and 43 public fields on all three trees, its only
differences being `execute`'s fourth parameter against `master` and `EventType::ALL`'s length against this branch;
`SPEC-MOK-002` rules 4 and 5 as amended are the authority for it, and it is stated here only because this paragraph is
where the three trees are compared.
