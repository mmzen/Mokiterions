...
|---|---|---|
| `mokiterions-tui/src/render.rs` | 17 | assert drawing internals; the module declares 47 private items — 30 functions and 17 constants — against 2 public ones, and 5 of the 17 additionally use a hook |
...
different trees. `WO-MOK-010`'s 21 additions and 0 removals are reconciled name by name in its `test-census.txt`,
which was captured at `4f32a9f` and reaches 190 rather than 200; that census is a capture and is re-taken against the
merge rather than edited. `WO-MOK-007`'s 7 additions and 0 removals are the seven named in rules 9 and 10 above,
which is where its own evidence left them uncounted.
