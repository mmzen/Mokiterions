...
[relations]
specifies = ["REQ-MOK-016", "REQ-MOK-017", "REQ-MOK-032", "REQ-MOK-033", "REQ-MOK-047"]
+++
...
table in either direction is a violation of `REQ-MOK-047`, and `SPEC-MOK-005` rule 8.4 is the check.
...
4. **`ADR-MOK-006` decision 11 is checked**: no entry may implement simulation semantics — the rules `SPEC-MOK-001`
   ...
   review, retained as a manual assessment under `VER-MOK-013`.
