...
**the figures are measured rather than estimated**, against the transcripts this repository commits. There are **two**,
and rule 11.3.1 is why neither can stand for the other: the first was written with nothing behind the port and the
second with a provider behind it. The first is
...
rule 11.7.2's, and it is what confirms that extrapolation. What is retained where is `VER-MOK-018`'s.
