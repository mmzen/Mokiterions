...
10.3a **The request object's field names, as amended 2026-08-28 under `WO-MOK-028`.** `protocol`, an integer
protocol version, `1` today; `tick`, the simulation tick; `actor`, the Mokiterion's identifier; `prompt`, the whole
composed prompt as one string; `model`; `reasoning`; and `schema`, the response schema rule 8.4 needs. Every field is
...
10.4a **The response object's field names, as amended 2026-08-28 under `WO-MOK-028`.** `protocol`; then exactly
one of `action` or `error`. `action` carries `verb` and, for a verb that takes one, `parameter`. `usage` carries
`prompt`, `cached_prompt`, `output` and `reasoning`, and accompanies `action`. `error` carries `kind` and `message`,
and rule 19.5a fixes the vocabulary of `kind`.
...
target acts on the connector path, the live-mode selection, the transcript path, the transcript output and the spend
ceiling — **five options, as amended 2026-08-28 under `WO-MOK-028`**, which added `--transcript-output`. The terminal
...
never records, so rule 18.4.2's refusal covers it as it covers the connector path and the ceiling.
