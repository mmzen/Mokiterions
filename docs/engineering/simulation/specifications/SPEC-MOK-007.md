...
target acts on `--connector-path`, `--live`, `--transcript-path`, `--transcript-output` and `--spend-ceiling`
— **five options**. `--transcript-output` was added 2026-08-28 under `WO-MOK-028`; the three that had never been
named were named the same day under `WO-MOK-029`. The terminal
