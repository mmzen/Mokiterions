...
- `ADR-MOK-005` decides the record sink's location — the binary target resolves and owns the file, the library target writes to a writer it is handed — the closed value alphabet that makes hand-written serialization provably total with an empty dependency table, and the extent of the public interface's growth, which is one optional parameter and no item. It is the deciding ADR for the 2026-08-20 amendment recorded above. **It supersedes nothing.** Engine authority, the trust boundary, the dependency direction and every determinism property stand as `ADR-MOK-001` records them.

## Decision assessment

The `decision_assessment` block in this document's frontmatter carries the assessment. Its `rationale` field has a governed length limit of 2,000 characters, and the 2026-08-20 amendment paragraph did not fit inside what remained. **The field therefore carries a one-sentence pointer to this section, and this section carries the paragraph in full.** Nothing was removed from the earlier text to make room, apart from the closing clause "covered by `ADR-MOK-001`, `ADR-MOK-002` and `ADR-MOK-003` together", which the amendment replaced with "covered by `ADR-MOK-001` through `ADR-MOK-005` together" and which is nine characters shorter. That substitution is `ADR-MOK-005`'s own required wording. This split is the implementation agent's decision of form under `WO-MOK-018`, disclosed here rather than left for a reader to notice, and it is the only respect in which the amendment departs from the instruction to append the paragraph to the field itself.

**Amended 2026-08-20.** The location of the record sink, the hand-written serialization strategy and the extent of the public interface's growth are decided by `ADR-MOK-005`. The binary target resolves and owns the destination file, while the library target writes to a writer it is handed and performs no filesystem operation. Records are serialized by hand against a closed value alphabet rather than by a dependency, because the engine package's dependency table admits no exception. The interface grows by exactly one optional parameter on the process-boundary function and by no item. That decision fires the already-declared **public-interface-or-protocol**, **technology-framework-vendor-or-external-service** and **material-alternatives** triggers. Engine authority, the trust boundary, the dependency direction and the determinism properties are untouched — the record projection mutates no state and draws no entropy — so the assessment stays `adr_required` and is covered by `ADR-MOK-001` through `ADR-MOK-005` together.
