...
at the commit recorded in `evidence/WO-MOK-018/baseline/COMMIT.txt`, because a capture taken afterwards could not
...
reproduced from `evidence/WO-MOK-018/gates.txt`: **246 tests, 246 passed, 0 failed, 0 ignored, 0 filtered out** across
...
**seven** `u64` counters in four struct fields. It is defect 4 of the nine in `evidence/WO-MOK-018/completion-summary.md`
...
| `W-HEX-001` observations | 2 — `WO-MOK-010`, `WO-MOK-011` | **3** — and `WO-MOK-018` |
| Active work | **1** — `WO-MOK-018` `[in_progress]` | **0 — none** |
...
identifier here belongs to the directory, `evidence/WO-MOK-018/`. Fifty-six files sit in it. `WO-MOK-010` and
...
**Disclosed rather than repaired.** `evidence/WO-MOK-018/analysis/amendments.py` — oracle 7 — carries `WO-MOK-018` at
...
reading *RESULT: FAIL — 1 finding*, the finding being *WO-MOK-018 carries status `implemented`, expected
`in_progress`*. Diffed against the retained report, two places move in 291 lines — the chain table's `WO-MOK-018` row, whose `ok` cell turns `**NO**`, and
...
`REQ-MOK-042` through `REQ-MOK-046`, `SPEC-MOK-006`, `ADR-MOK-005`, `VER-MOK-012`, `WO-MOK-018`, and `VREC-MOK-012`
...
Under `docs/engineering/simulation/evidence/WO-MOK-018/`, everything `VER-MOK-012`'s *Evidence retention* section
lists. In particular:
