...
state, and its prohibition on any engine dependency are all in force and are checked under `VER-MOK-010`. **The
...
`REQ-MOK-040` and `REQ-MOK-041` being approved by the product owner, and on `VER-MOK-010` being approved by the
...
worktree at the commit recorded in `evidence/WO-MOK-010/baseline/COMMIT.txt`, because a capture taken afterwards could
...
`REQ-MOK-041`, `VER-MOK-010`, `WO-MOK-010`, and `VREC-MOK-010` when the verification record is written. The gap this
...
**That this specification needs no amendment is a claim, and `VER-MOK-010` oracle 5 checks it by comparing the interface
...
`VER-MOK-010`, in full: its five oracles, its requirement-to-evidence matrix, its property and invariant tests, its
...
Under `docs/engineering/simulation/evidence/WO-MOK-010/`, everything `VER-MOK-010`'s *Evidence retention* section
lists. In particular:
...
11. residual uncertainty, including everything under *Stop and escalate conditions* above that remains open, and the
    statement that `VREC-MOK-010` is a separate commit-bound record that this work order does not write and cannot
    self-approve.
