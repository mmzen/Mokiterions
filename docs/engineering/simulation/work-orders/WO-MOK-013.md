...
**This work order must not be approved while the open decision in *Authorized decision envelope* stands.** It is not a
decision the implementation may take, and it is not one approval can leave to be discovered: one of its two branches
amends an approved product requirement and an approved verification contract, and the other amends a specification
rule. Approving the work order without taking it would authorize an implementation that must violate either
`REQ-MOK-020` or `REQ-MOK-047`, both of which would then be approved.
...
6. **Whichever of the two amendments the open decision selects** — either `REQ-MOK-020` and `VER-MOK-005`, or
   `SPEC-MOK-003` rule 5's log rows. Not both, and not neither.
7. **The evidence `VER-MOK-013` contracts**, under `evidence/WO-MOK-013/`.
8. **A roadmap entry** for this chain.
...
- **Any pane threshold**, except the log's row count if and only if the open decision selects that branch. The roster's
  `W ≥ 100`, the inspector's `W ≥ 140`, the log's `H ≥ 38` and the floor's `34 × 22` are untouched either way.
...
### The open decision — product owner, and it blocks approval
...
- Touches one approved artifact, in one role. **Monotonicity is unaffected**, since no pane's presence changes.

**This option was not among the three the owner was shown on 2026-08-20** and is reported now because it is the only
arrangement satisfying `REQ-MOK-047` and `REQ-MOK-020` together.

**What the engineering owner would recommend, with the decision remaining the product owner's.** Option B. The log is
...
**One risk under option B, stated rather than left to be found.** `12 × 3 = 36` fills a 36-row interior exactly, with
no slack. `SPEC-MOK-001` bounds the population at twelve and calls a name table longer or shorter than the population
...
| Observer layout | Untouched under option A; the log's row count under option B |
| Observer tests, both tiers | New cases per `VER-MOK-013`'s matrix; existing cases asserting the two-cell arithmetic corrected |
| `SPEC-MOK-003` | Rule 4 and rule 5 amendments below, plus one under option B |
| `SPEC-MOK-004` | Rule 11 figures, arrival by arrival |
| `REQ-MOK-020`, `VER-MOK-005` | Under option A only |
| `docs/ROADMAP.md` | An entry for this chain |

The named anchors — `BAR_ROW_OVERHEAD` at `render.rs:66`, `bar_width` at `render.rs:546`, `rows_per_entry` at
`render.rs:429`, the announcement's `Span::raw` at `render.rs:178`, `announcement_text` at `render.rs:187` and
`ROSTER_WIDTH` at `layout.rs:23` — were measured at `ff3a155` while authoring this work order and are stated as where
to start, not as the full extent.
...
6. **Rule 5, the log's row count — option B only.** As set out in the open decision.

Each is put to the technical owner as its own question, enumerating its provisions, at approval. A single answer
covering all five would be an approval by implication of four.
...
- **A closing review** recording every decision with the role that took it, on the `evidence/WO-MOK-012/closing-review.md`
  pattern, including the open decision above and the option chosen.
...
1. **The open decision above, if it has not been taken.** Do not begin.
2. **Any further conflict between an approved artifact and one of the three requirements**, of the kind
   ...
   not to pick the reading that lets the work proceed.
...
1. The option taken on the open decision, the role that took it, and the instruction verbatim.
