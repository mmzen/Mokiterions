...
completed.** `INT-MOK-010` and `CAP-MOK-010` moved to `approved` on that authorization, nothing being outstanding above
them. The ten requirements, `VER-MOK-016` and this work order did not, for a reason the harness states and this packet
...
amendments, `REQ-MOK-051` through `REQ-MOK-060`, `VER-MOK-016` and this work order in one act. `validate` reports PASS
with 116 artifacts and no errors or warnings, and `preflight . --work-order WO-MOK-016 --phase start` reports **PASS with
...
could not have been: `VER-MOK-016` oracle 1 requires the engine's output captured *before* any change, from a
...
per-cell digest with the stated subset kept whole, under `evidence/WO-MOK-016/baseline/`. Its provenance is measured
...
is made against the implemented code at approval and recorded as `VER-MOK-016` manual assessment 9, an unrecorded
...
`evidence/WO-MOK-016/escalation.md`, which is the record they were decided against and is not rewritten after the fact.

The first was stop conditions 5, 7 and 11: `REQ-MOK-058`'s lethality bound was missed on every declared seed and
...
covariate was masking it. `WO-MOK-016` reserves exactly this — "the first is a finding about turn order and is the
...
`VER-MOK-016`'s amendment record of 2026-08-20 carries the four provisions and the approval; `identifier.md` is the
...
waste condition at all, so no change there can reach it. That is what makes `INT-MOK-010`'s promise of a byte-identical
`baseline` keepable, and `REQ-MOK-060` fixes the permitted set for that reason.
...
| The source's decision rule | Survival before society: five ordered branches, answering an unanswered attack first, then rule 19's cases 1 and 2, then contact, then perception, then rule 19's remainder. **Amended 2026-08-20 to six branches** — rule 19's case 3 hoisted ahead of contact and perception, its case 4 left last — on the measurement recorded in `evidence/WO-MOK-016/escalation.md` | technical owner |
...
| `REQ-MOK-058`'s survivor floor | Lowered from six to five, before any measurement, because of the damage decision above. **Ratified at five on 2026-08-20** on the first measured curve, against measured alternatives of three and two | product owner |
...
a striker pressing an attack through to a kill, and `REQ-MOK-053` records that the alternative — a cost scaling with the
damage dealt — was declined because it would likely have made `REQ-MOK-058`'s lethality bound unreachable. What holds the
...
yields `VER-MOK-016`'s oracle 8 for nothing.
...
the stream for a decision that was never used. `REQ-MOK-057` states this and this work order forbids the restructuring
that would reintroduce it.

**Lowering the floor to five contradicts the argument that produced six, and `REQ-MOK-058` says so.** Six was half the
...
arithmetic, and the ratification at `VER-MOK-016` assessment 4 is where that is settled against a measured curve. Because
`REQ-MOK-058` is still `draft`, the move carries no amendment-record row: it was an edit to an unapproved obligation.
...
`encounter_resolved` carrying the verb in its detail. The reason given for three is `CAP-MOK-010`'s own driver for new
...
`action_trace` already reports everything they change. **`REQ-MOK-052` therefore takes no row in `SPEC-MOK-003` rule 11's
authority table**, and provision 1 of that specification's amendment is corrected from the four requirements it named to
three: `REQ-MOK-053` for `attack_resolved`, `REQ-MOK-055` for `threat_resolved`, `REQ-MOK-056` for `surrender_resolved`.
...
`CAP-MOK-010` excludes absolutely and `REQ-MOK-060` holds byte-identical. The live alternative was growing the list only
...
`REQ-MOK-058`'s floor and `REQ-MOK-060`'s ceiling are ratified or amended on measurement (`VER-MOK-016` assessments 4 and
...
approved together with `REQ-MOK-051` through `REQ-MOK-060`, `VER-MOK-016` and this work order in one act, and only then
...
2026-08-20, and the repository owner approved it on the same day together with `REQ-MOK-051` through `REQ-MOK-060`,
`VER-MOK-016` and this work order, in the single act this section requires.
...
numeric mechanism that `REQ-MOK-060` leaves open on measurement, so it is two amendments and only the first is written;
...
**Measuring it found a third defect, and this one was an obligation rather than an enumeration.** `REQ-MOK-057`'s
five-branch ordering was implemented exactly as approved and measured to be **unsatisfiable** against `REQ-MOK-058`: rule
...
`REQ-MOK-058`'s floor ratified at five. `SPEC-MOK-001` rule 26, `REQ-MOK-057`, `REQ-MOK-058` and `VER-MOK-016` each carry
...
unsatisfiable, and nothing short of running the world would have shown it — which is what `REQ-MOK-058` exists to force and
...
1. **The `suffered` field is present only when the record is non-empty.** Forced: an unconditional field changes every
   `baseline --trace-actions` line and `CAP-MOK-010` excludes that.
...
8. **`REQ-MOK-060`'s numeric form is deferred to a second amendment**, per the correction above. Confirmed below.
...
targeted line exists nowhere but under `social`. `VER-MOK-016` carries both as checks, one of them a static check over
...
1. *Scope*. No longer excludes social interaction between Mokiterions, and names `CAP-MOK-010`. The exclusion of
...
6. *Data and interface contracts*. The observation gains the observer's own `fear` and the suffered-attack record, and
   ...
   **Corrected 2026-08-20 while the text was written: this provision also fixes the `action_trace` line's shape, which
   ...
   derived, not chosen: an unconditional field would change every `baseline --trace-actions` line, and `CAP-MOK-010`
...
7. Rule 3, *Observation*. The sentence "`fear` is deliberately **not** carried: no rule and no decision source reads it"
   ...
   which requirement obliges the reader that makes them non-inert — `REQ-MOK-054` for the fields and `REQ-MOK-057` for
...
9. Rule 5, *Reference decision*, and rule 19, *Trait-aware decision*. Rule 5's accumulation paragraph is amended: its
   ...
   direction**" are both replaced by a reference to `REQ-MOK-060`'s stated ceiling, and both are retracted rather than
   ...
   **Corrected 2026-08-20 while the text was written: this provision is two amendments and not one, and only the first
   ...
   settled. It is not: `REQ-MOK-060`'s *Open decisions* leaves the mechanism to the technical owner **on measurement**,
...
12. Rule 12, *Survival decay and fear*. Its own computation is unchanged. Its closing sentence — "**No rule reads
    ...
    **inverted**: a consumer now exists, it is the source of `REQ-MOK-057`, and the appended threat rule writes another
...
1. Rule 5's enumeration gains the fourth `Policy` variant, `Action`'s seven target-carrying variants, and the three
   ...
   **Corrected 2026-08-20 against the source: this provision stated a fact about the engine that does not hold.** It
   ...
   justification for a private struct and understated the one genuine growth, `EventType::ALL`'s length. `VER-MOK-016`'s
...
3. **Rule 5's growth table gains a fourth row, and provision 1's enumeration was incomplete without it.** The
   ...
   `fear`. Growth under `CAP-MOK-010` becomes `1 + 7 + 3 + 3 + 1`, and the table's own count moves from three items
   ...
   **Written 2026-08-20, after provisions 1 and 2 were approved, and approved in a separate act.** The gap is worth
   ...
   provision.** `VER-MOK-016`'s interface-growth check is re-run against four rows rather than three.
...
1. Rule 11's authority table gains **three** rows, one per new event type, each mapping to the requirement that
   authorizes it — `attack_resolved` to `REQ-MOK-053`, `threat_resolved` to `REQ-MOK-055`, `surrender_resolved` to
   `REQ-MOK-056` — because that rule requires every event type to map to an authorizing requirement, and
   `mokiterions-tui/src/authority.rs` covers `EventType::ALL` exhaustively. **`REQ-MOK-052` takes no row**, on the
...
- **`REQ-MOK-005`** — its four-verb enumeration is re-read as the *core* set, beside which `REQ-MOK-052` places the
...
- **`REQ-MOK-034`** — its survivor floor is re-measured on the same terms, and its clause that the `reference` and
  `baseline` sources' outcomes "are frozen" is **narrowed to `baseline`**, because `REQ-MOK-060` moves `reference` by
  construction. This is the one amendment in this list that changes an obligation rather than re-measuring one, and
  `VER-MOK-016` manual assessment 6 is where the owner accepts it.
...
- Every check, measurement and manual assessment `VER-MOK-016` requires, and the evidence it retains.
...
- The contact radius, the set of eleven action kinds, or any precondition in `REQ-MOK-052`.
- Where the composition correction is placed. The permitted set is fixed by `REQ-MOK-060` and rule 4, rule 9's eat
...
- Any change to a floor, a threshold, a default or an exit code, including `REQ-MOK-058`'s floor of five, `REQ-MOK-060`'s
...
`VER-MOK-016`, in full: its eight oracles, its requirement-to-evidence matrix, its stated monotonicity band, its
...
Under `docs/engineering/simulation/evidence/WO-MOK-016/`, everything `VER-MOK-016`'s *Evidence retention* section lists.
In particular:
...
- `world-rules-unchanged.txt`, the line-for-line comparison showing rule 4, rule 9's eat effect, the food table and rules
  14 to 16 unchanged;
- `reads.md`, the enumeration `REQ-MOK-059` requires, with rules 14 to 16 and 18 recorded as outside the obligation and
  why;
...
7. **A verb never applies** in any run across the declared matrix, or a defender's branch distribution is degenerate on
   every seed. The second is a finding `INT-MOK-010` anticipates and wants reported, not hidden by tuning the source.
...
10. **A read cannot be resolved** to the acting Mokiterion's own state, its observation or a named individual — that is,
    `REQ-MOK-059` cannot be satisfied without redesigning something.
...
- **`VREC-MOK-010` records `VER-MOK-010` as satisfied but for its `VREC-MOK-005` gate row.** That gate — the outstanding
  amendments and manual assessments under `VREC-MOK-005` — is a precondition of this work under `VER-MOK-016` oracle 7,
