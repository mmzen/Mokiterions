...
created = "2026-08-18"
updated = "2026-08-18"
commit = "9d9641fee6a1773ac86170930cf0073500a8b4cd"
git_object_format = "sha1"
worktree_state = "clean"
verified_at = "2026-08-18T06:52:20Z"
artifact_snapshot_sha256 = "85184aaa2888a30cb4f872ceb5423138b54a1631e210dd188fc0a037932c3b91"
...
This ready record binds retained evidence for `WO-MOK-005` to candidate commit `9d9641fee6a1773ac86170930cf0073500a8b4cd`. An accountable assurance owner must review the evidence and transition the record to `verified`; this command did not approve, commit, tag, release, or publish anything.
...
`9d9641fee6a1773ac86170930cf0073500a8b4cd`, **every automated case, invariant, static check,
...
`completion-summary.md` discloses fifteen findings and two further notes. None is a failure against
`VER-MOK-005`; each is stated so that verification, if given, is given knowingly. The eleven that
bear on how much the green gates mean:
...
11. **All 109 observer tests are in the internal tier, and the two documents governing that choice
    point opposite ways.** `SPEC-MOK-003` grants the implementation "test organization, fixtures and
    helpers" under *Explicitly unspecified decisions*, and the observer builds one target — a binary —
    so `mokiterions-tui/tests/` cannot reach the code: a Rust integration test links a library target
    and the observer has none. `verification.rs`, the entire `VER-MOK-005` cross-cutting suite at
    1,255 lines, is therefore `#[cfg(test)] mod verification` inside the binary. `REPOSITORY_CONTEXT.md:29`
    states the opposite as a repository constraint, citing `SPEC-MOK-002` rules 7 to 10, whose scope
    as amended reads "Every rule below is a rule about the engine package". The implementation took
    the specification's grant; the result is one package with two test tiers and one with none.
    Closing it needs a library target for the observer, which changes what `SPEC-MOK-003`'s component
    layout fixes and which that same list says the implementation may **not** choose — so it needs an
    approved requirement and an amendment, and belongs to a work order that has them rather than to
    this one. No item would be widened to `pub` to achieve it.
