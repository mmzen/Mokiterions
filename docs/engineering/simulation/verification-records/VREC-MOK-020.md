...
created = "2026-08-22"
updated = "2026-08-22"
commit = "a3a65abdd9fef332ba0312fbae9dccd73e610950"
git_object_format = "sha1"
worktree_state = "clean"
verified_at = "2026-08-22T09:21:49Z"
artifact_snapshot_sha256 = "424aef743bd199efd765f723b35af5cd88f1304fd96f8e2e8d9298fe0c179bac"
...
This ready record binds retained evidence for `WO-MOK-020` to candidate commit `a3a65abdd9fef332ba0312fbae9dccd73e610950`. An accountable assurance owner must review the evidence and transition the record to `verified`; this command did not approve, commit, tag, release, or publish anything.

The record is intentionally created after the candidate commit it names, avoiding self-referential commit metadata.

## What this record claims

`WO-MOK-020` is `implemented` and `VER-MOK-004` is `approved`, as amended twice on 2026-08-22. At candidate
commit `a3a65abdd9fef332ba0312fbae9dccd73e610950` all nine items of the work order's *Required verification*
...
**This record is the third capture under this identifier, and the first two are named rather than quietly
replaced.** They bound commits `1b4ee1e` and `9153f35`. The first was overtaken by a correction to
`usage-text.md`, which would have falsified its binding had the file been edited under it; the second by
amendment 6, ratified after it was taken. A verification record cannot be re-pointed at a later commit and
the harness has no `superseded` status for one, so each was deleted and re-taken. Both stood at `ready` and
**neither was ever accepted by the assurance owner**, so nothing accepted was withdrawn. Act 10 of
`WO-MOK-020`'s *Decision record* is the owner's direction for this one.
...
The four are fixed by **amendment 6** of `WO-MOK-020`, ratified on 2026-08-22 by the owner in the assurance
...
is the stated consequence of act 7, and it is disclosed here and in `WO-MOK-020` rather than left for a
reader to find.
...
The three harness readings were taken a fourth time on the tree that carries amendment 6 and returned the
same figures again, which is what should happen: the amendment changed five markdown files and no artifact
the harness reads for structure. `gates.md`'s opening paragraphs record all four capture points.
...
2. **The code gates were taken at commit `6d6be40`, not at the commit this record binds.** The six commits
...
9. **Three files of the retained evidence were edited after amendment 6 and before this capture** —
   `gates.md`, `README.md` and `usage-text.md` — to say that the rows are amended rather than outstanding.
   **No measurement in any of them changed.** The figures were captured before the amendment was drafted, and
   what was added is dated ratification notes that leave the original observations readable. The two
   discarded captures are the audit trail: their bindings are in the git history of this file.
