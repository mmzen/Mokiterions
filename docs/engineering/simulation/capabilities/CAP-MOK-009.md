...
- `REQ-MOK-042` — contact, and the encounter the engine detects from it.
- `REQ-MOK-043` — the engine applies the seven new actions, beside `REQ-MOK-005`'s four rather than replacing it.
- `REQ-MOK-044` — combat resolution: deterministic cost to both parties, and death by combat.
- `REQ-MOK-045` — the deferred response: `fear` and attacks-suffered on the observation, and the defender's own answer.
- `REQ-MOK-046` — `threaten` raises the target's `fear` and does nothing else.
- `REQ-MOK-047` — `surrender` forfeits satiety to the attacker and ends the encounter.
- `REQ-MOK-048` — the `social` decision source, which reads `fear`.
- `REQ-MOK-049` — habitability under that source: the survivor floor.
- `REQ-MOK-050` — no population aggregate is read by any rule, source or validation path.

- `REQ-MOK-051` — the class composition of a territory's standing resources stays within a stated ceiling.

Ten rather than fewer, because each states an obligation that can fail on its own and be measured on its own, and
because they do not share an accountable owner: `REQ-MOK-044`, `REQ-MOK-047`, `REQ-MOK-049` and `REQ-MOK-051` are
product judgements about how violent and how survivable this world is, while `REQ-MOK-045` and `REQ-MOK-050` are
...
outcomes "are frozen": `REQ-MOK-051` moves `reference` and `individual` by construction, so that clause is narrowed to
...
`REQ-MOK-043` places the targeted set. **It is amended and not superseded**, deliberately: it is cited by
