...
An attack that cannot kill is a gesture. `INT-MOK-009` states death by combat as a desired outcome and as something
...
The cost of determinism is stated plainly and is the reason `REQ-MOK-049` and the identifier-monotonicity bound exist.
...
hundred strikes, and at the contact rate `REQ-MOK-032`'s arithmetic gives, `REQ-MOK-049`'s one-death-per-seed bound would
likely have been unreachable. **And the `5` cost is deliberately a weak brake**: it does not stop a striker pressing an
attack through to a kill, which places `REQ-MOK-049`'s survivor floor on the `social` source's willingness to attack rather
than on the cost of attacking. That was the owner's choice with the trade named, and it is why `REQ-MOK-048`'s
survival-first ordering is load-bearing rather than a stylistic preference.

## Preconditions and trigger

The trigger is the engine applying a valid `attack` or `fight` under `REQ-MOK-043`, after every precondition of
`REQ-MOK-042` and `REQ-MOK-043` has been checked against authoritative state.

Both Mokiterions are living at the moment of resolution. A target that died earlier in the same tick yields a rejection
under `REQ-MOK-043` and never reaches this rule.
...
- **Damage is an integer function of the striker's `energy` and `health` at the moment of resolution, and of nothing
  ...
  Mokiterion a weaker attacker without any Mokiterion reading another's strength, which `CAP-MOK-009` excludes.
...
- **The resolution is reported as one event carrying both parties' transitions** — the striker's identifier, the target's
  ...
  target died. This is the reporting obligation `INT-MOK-009` records as a risk: rule 2 runs each Mokiterion's whole
...
- **Death by combat is distinguishable in the stream from death by starvation or exhaustion**, so that the count
  `REQ-MOK-049` and `VER-MOK-012` measure can be taken from the run's own output rather than inferred.
...
- A striker at `energy` zero still deals `10` damage, the function's minimum, and pays a cost that saturates at zero. Being
  ...
  if the owner later wants one, that is a change to `REQ-MOK-043`'s precondition list, not to this rule.
...
- A striker that kills its target still pays its own cost, and the tick continues. There is no follow-up action, no free
  move onto the target's cell, and no resource transfer — `REQ-MOK-047` is the only rule that moves satiety between
...
- **No population aggregate is read**, including no count of living Mokiterions and no count of ongoing encounters.
  `REQ-MOK-050` states this generally.
- **No damage is dealt outside a resolved `attack` or `fight`.** `threaten` deals none (`REQ-MOK-046`), `surrender` deals
  none (`REQ-MOK-047`), `approach` and `avoid` deal none, and there is no collision damage, no passive attrition between
