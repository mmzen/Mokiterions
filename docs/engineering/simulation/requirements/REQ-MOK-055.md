...
nobody, and its whole effect is on the target's `fear` — which is the attribute `REQ-MOK-054` puts on the observation and
`REQ-MOK-057`'s source reads. That makes `threaten` the only action in this world whose effect is entirely a change to
...
an effect will be proposed often by any source that can propose it, and `VER-MOK-016` measures how often. The owner's
...
`VER-MOK-016`'s evidence readable. And it composes deliberately with `REQ-MOK-057`'s thresholds: that source attacks a
...
The trigger is the engine applying a valid `threaten` under `REQ-MOK-052`: both Mokiterions living, in contact under
`REQ-MOK-051`, target not the actor, all checked against authoritative state.
...
- **`threaten` does not populate the suffered-attack record of `REQ-MOK-054`.** A threatened Mokiterion may not `fight`,
...
- **The threat is reported as one event carrying the transition it caused** — the threatener's identifier, the target's
  identifier, the increase applied and the target's resulting `fear`. As with `REQ-MOK-053`, this is what keeps the
...
- A threat against a Mokiterion that died earlier in the same tick is rejected under `REQ-MOK-052`.
- A threat at distance `2` or more is rejected on contact under `REQ-MOK-051`.
