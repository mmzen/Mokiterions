...
moved it is `REQ-MOK-053`'s damage decision taken in the same session: a range of `10..=30` kills a full-health target in
...
text: `VER-MOK-016` records survivors per seed whether or not the floor is met, so the evidence will show how much
headroom five actually bought, and a curve that clusters at five or six is itself a finding about the damage decision.

**At least one death by combat.** `INT-MOK-010` states that an attack which cannot kill is a gesture, and
`REQ-MOK-053` makes death by combat reachable. Reachable is not reached. This obligation is what distinguishes a
mechanism that exists from a mechanism that happens, and it is stated per seed rather than in aggregate so that it cannot
be satisfied by one violent seed among many. `REQ-MOK-053` obliges the stream to distinguish death by combat from death
...
it or anything like it. `REQ-MOK-059` states that separation, and it exists partly because this requirement makes the
...
`REQ-MOK-057`, on a seed in the declared verification set.
...
- **At least one death in the run is attributable to combat**, on every declared verification seed, distinguishable in the
  event stream from death by starvation or exhaustion under `REQ-MOK-053`.
...
- **The `social` source's own ordering and thresholds are the one legitimate lever**, because they are that source's
  behavior rather than a rule's constant. `REQ-MOK-057`'s ordering — survival first, then a perceived meal, and only then
  society — is what this floor rests on, given that `REQ-MOK-053`'s `5` `energy` cost brakes combat weakly. Changing that
  order or those thresholds is a change to `REQ-MOK-057` and is governed there; it is not a tuning commit, and it is not
  ...
  **Corrected 2026-08-20.** This bullet described `REQ-MOK-057`'s ordering as "a Mokiterion acts socially only when it is
  neither hungry nor tired". **That was not what `REQ-MOK-057` specified.** Its branch 2 fired on a tolerated resource
  *underfoot* or on exhaustion, and its seek-move sat behind both social branches, so a Mokiterion hungry with food four
  ...
  reworded here: `REQ-MOK-057`'s amendment of the same date hoists the seek-move above society, so the sentence is now
...
- **No population aggregate is read inside the simulation** to satisfy this requirement, and nothing about a survivor count
  is available to any rule, source or proposal validation. `REQ-MOK-059`.
...
- **Meeting this floor must not be achieved by any change to the `baseline` source.** Its outcomes are held
  ...
  `reference` too; that clause is amended by `REQ-MOK-060`, which moves `reference` by construction.
