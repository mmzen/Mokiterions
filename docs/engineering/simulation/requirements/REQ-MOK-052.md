...
- **No population aggregate is read** by any precondition, target selection, ordering or tie-break. `REQ-MOK-059` states
  this generally.
...
**Then** the action is applied exactly once, its costs are applied to both parties as `REQ-MOK-053` specifies, and one
...
- Whether `approach` and `avoid` emit their own event kind or reuse the movement path's reporting **was** the technical
  ...
  the three is authorized by this requirement — `attack_resolved` by `REQ-MOK-053`, `threat_resolved` by `REQ-MOK-055`,
  `surrender_resolved` by `REQ-MOK-056`. It is no longer open, and it remains `SPEC-MOK-001`'s and `SPEC-MOK-002`'s to
  state as text.
