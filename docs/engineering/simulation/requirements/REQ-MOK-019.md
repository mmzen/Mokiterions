...
- every standing resource, at its authoritative coordinates, with its class distinguishable at the fidelity
  `SPEC-MOK-003` declares for per-resource class, and with each territory's per-class counts available at every
  fidelity;
...
- The rendered fidelity, glyph and colour assignment, coordinate mapping, orientation, shared-position
  precedence, and zoom behavior are fixed by `SPEC-MOK-003`.
...
which the view presents a region rather than the whole world are fixed by `SPEC-MOK-003`.
