...
What a Mokiterion does when it is attacked is the most interesting decision in this world, and `INT-MOK-010` states that
...
interaction memory, which `CAP-MOK-010` excludes: nothing records *who has ever* attacked whom, only what has landed
...
inputs to the identifier-advantage risk `VER-MOK-016` bounds.

**No inert value.** `SPEC-MOK-003` rule 4.5 refused a field nothing reads, and `fear` was the precedent for that refusal.
Both fields added here are read by the source `REQ-MOK-057` introduces, at the same opportunity they are carried on. If
...
- **The observation carries the observing Mokiterion's own `fear`**, as its current bounded value in `0..=100`, alongside
  ...
  Mokiterion's; nothing on the observation exposes another Mokiterion's attributes, which is `CAP-MOK-010`'s exclusion of
...
- **`fight`, `retreat` and `surrender` are valid only when that record is non-empty**, and only against a Mokiterion named
  in it. This is `REQ-MOK-052`'s precondition and this requirement supplies the state it is checked against.
- **`retreat` moves the defender one cell away from the named attacker**, on the `avoid` contract of `REQ-MOK-052`: one
  ...
  no encounter, because `REQ-MOK-051` defines contact as a relation over positions and not as a state to be exited.
...
- **Nothing else reads the record.** It is not an input to damage, not an input to contact, not an input to survival
  decay, and not reported as an attribute. The resolution events of `REQ-MOK-053` already report what struck whom; this
...
- A Mokiterion attacked twice before its next opportunity may answer only one attacker — the one it names — and the
  window closes on both. Answering is one action, and the closed contract of `REQ-MOK-052` applies exactly one action per
  opportunity.
- A Mokiterion attacked by a Mokiterion that has since died may still propose `fight` naming it; the proposal is rejected
  under `REQ-MOK-052`'s living-target precondition, the opportunity is consumed, and the window closes.
...
- No source is obliged to read either field. `baseline`, `reference` and `individual` continue to ignore what they ignore
  today, and carrying a field they do not read is not an inert value while `REQ-MOK-057`'s source reads it.
...
- **No entropy draw** is taken to build an observation, clear a window, or resolve a `retreat` beyond what a move already
  takes — which is none.
- **No population aggregate** is carried or read. `REQ-MOK-059`.
- **No memory beyond the window.** No count of lifetime attacks, no last-attacker field that outlives the opportunity, no
  grudge. `CAP-MOK-010` excludes it and this field is bounded so that it cannot become it.
...
**Given** a run under the source of `REQ-MOK-057`
