...
because it is the assurance owner's rather than the product owner's. `REQ-MOK-043` through `REQ-MOK-048` each repeat a
...
**And the temptation is now concrete rather than hypothetical.** `REQ-MOK-049` states a survivor floor. The cheapest way
to meet a survivor floor is to have the engine suppress combat when the population falls low — which is
`if food < threshold: start_war()` with the sign reversed, and would pass every test `REQ-MOK-049` states while
...
*outside* the simulation: `REQ-MOK-014`, `REQ-MOK-034` and `REQ-MOK-049` are all survivor counts, `VER-MOK-012` counts
...
- **A behavioral rule reads the acting Mokiterion's own state and, where the rule acts on a second Mokiterion, that
  Mokiterion's own state as the rule's stated inputs require.** `REQ-MOK-044`'s damage reads the striker's `energy` and
  `health`; `REQ-MOK-046` writes the target's `fear`; `REQ-MOK-047` moves `satiety` between two named Mokiterions. Those
...
- **The verification and observation paths may read aggregates freely**, and this requirement does not restrict
  `REQ-MOK-014`, `REQ-MOK-034`, `REQ-MOK-049`, `VER-MOK-012`, the event stream's terminal summary, or the observer. It
...
**Then** it violates this requirement, and it does so even though it would raise the survivor count `REQ-MOK-049`
measures.

### Example: measurement is unaffected

**Given** `REQ-MOK-049`'s survivor floor and `VER-MOK-012`'s deaths-by-cause count
