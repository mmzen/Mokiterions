...
`CAP-MOK-010` excludes on both sides.

**"Settles" is the honest verb, and "ends the encounter" needs to be read narrowly.** There is no encounter object to
end. `REQ-MOK-051` defines contact as a relation recomputed from positions, and `CAP-MOK-010` excludes interaction
memory, so there is nothing to hold a truce in. What a surrender ends is the answer: the transfer is the whole
settlement, the suffered-attack window closes under `REQ-MOK-054`, and neither Mokiterion carries any further obligation
...
The trigger is the engine applying a valid `surrender` under `REQ-MOK-052`: the acting Mokiterion is living, has an
attack in its suffered-attack record under `REQ-MOK-054`, and names as its target a living Mokiterion in that record.
...
- **No `fear` is written by a surrender**, in either direction. Rule 12's own write still applies to each Mokiterion at
  its own opportunity, and `REQ-MOK-055` is the only other rule that writes `fear`.
- **The suffered-attack window closes**, as it does for any answer, under `REQ-MOK-054`. The surrendering Mokiterion may
...
- A surrender naming a Mokiterion that has since died is rejected under `REQ-MOK-052`'s living-target precondition, the
...
- **No entropy draw.**
- **No population aggregate read.** `REQ-MOK-059`.
- **This is the only rule that moves any attribute from one Mokiterion to another.** Damage under `REQ-MOK-053` reduces
  without crediting anyone; a threat under `REQ-MOK-055` writes without spending. Transfer is unique to surrender and the
  set is closed.
- **No per-pair state.** No truce, no cooldown, no record that this pair has settled. `CAP-MOK-010`'s exclusion of
...
**Then** the attack is valid and resolves under `REQ-MOK-053`.
...
- Whether a bounded truce should exist at all is deferred, not left open in the implementation. It is declined here for its
  cost in per-pair state, and `VER-MOK-016`'s measurement of repeat surrenders between the same pair is the evidence a
