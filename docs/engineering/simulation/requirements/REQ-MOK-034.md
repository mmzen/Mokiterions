...
| 2026-08-20 | Narrowed the frozen-outcome constraint from "the reference or baseline source" to `baseline` alone, because `REQ-MOK-051` corrects `SPEC-MOK-001` rule 5's waste condition and that rule is the reference source's own proposal logic. **The floor of eight of twelve is not touched by this row**, and neither is the density, the horizon, the seed set, or the range-narrowing clause. | **OUTSTANDING.** Requires the product owner. This amendment must be approved **before** the change that moves the reference source lands, and it need not precede `WO-MOK-012`'s own approval; it is stated in full in that work order's *Required amendments* section. The implementation agent wrote the text and did not decide the substance: narrowing this clause rather than declining `REQ-MOK-051` was the product owner's decision of 2026-08-20, recorded in `WO-MOK-012`. **The floor itself is re-measured under `WO-MOK-012` against the corrected world and is a separate later act**, taken only if the measurement moves it, because re-measuring it requires the change to exist. |
...
- Meeting this floor must not be achieved by any change to the baseline source. Its outcomes are frozen under
  `REQ-MOK-033` and held byte-identical under `CAP-MOK-009`.

  **Narrowed 2026-08-20 under `REQ-MOK-051`.** This constraint read "any change to the reference or baseline source.
  Their outcomes are frozen under `REQ-MOK-033`", and it named one source too many. `REQ-MOK-051` corrects the waste
