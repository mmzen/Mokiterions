...
| 2026-08-22 | **No case, pass condition, viewport set, seed set or evidence path changed, and no row of the requirement-to-evidence matrix moved. The *Residual uncertainty* entry for `REQ-MOK-027` records that the residual it disclosed has been accepted by the product owner rather than resolved.** The row of 2026-08-22 above is not touched: it named the residual as the product owner's and recorded it below rather than resolving it, and that is what the entry below now answers. The entry keeps every sentence it had — including that these cases measure the concession and not the requirement, and that no PASS from this contract may be read as `REQ-MOK-027` satisfied at every viewport — and gains what the acceptance settles: that the floor is not raised, the accepted seed range is not narrowed and the requirement is not amended, that the residual therefore stands rather than closes, and that the acceptance is void if the floor, the accepted seed range or `SPEC-MOK-003` rule 8's field set changes. **The contract's count of automated rows is unchanged at 96** and every case added under `WO-MOK-008` still measures conformance to rule 8 as amended. Clause 4's first row, which the 2026-08-22 row above named as not yet ratified and warned would change this contract's added case if the position moved, was **ratified as written** on the same date, so that case is unaffected and the warning is discharged rather than acted on. | **The acceptance is the repository owner's act of 2026-08-22, taken as product owner** — the role this contract and `SPEC-MOK-003` both name for it — with the three courses those artifacts enumerate put alongside it and the cost of each measured. The ratification named above is the same owner's act as accountable technical owner, recorded in `SPEC-MOK-003`'s amendment row of that date rather than here. This row is written by the implementation agent, which recorded both acts and decided neither, and which changed no case, figure or path in this contract. Both acts were taken after `WO-MOK-008` merged at `2ecb78a`, so **`VREC-MOK-021` does not cover them**: it is `verified`, bound to `3da6acc`, conforms to this contract as it read at that commit, and cannot be re-pointed. `VREC-MOK-005`, which binds this contract, is `ready` rather than `verified`, so no verified record asserts the *Residual uncertainty* section in its former wording. |
...
- **`REQ-MOK-027` is not satisfiable at the floor, and these cases measure the concession rather than the
  ...
  **Accepted, not resolved.** The product owner accepted it knowingly on 2026-08-22, and the residual therefore stands
  rather than closes. None of the three courses was taken: the floor is not raised, the accepted seed range is not
  narrowed, and `REQ-MOK-027` is not amended. What the acceptance changes is that the choice among them has been made
  and this residual no longer awaits the product owner. What it does not change is the arithmetic, any case in this
  contract, or the reading in the sentence above — a PASS here still does not mean `REQ-MOK-027` is satisfied at
  `34 × 22`, and the acceptance is void if the floor, the accepted seed range or rule 8's field set changes. It is
  recorded normatively in `REQ-MOK-027`'s *Open decisions*, on the precedent of `REQ-MOK-014`'s deferred matter, and
  in `SPEC-MOK-003`'s amendment row of the same date, which also carries the technical owner's ratification of clause
  4's first row.
