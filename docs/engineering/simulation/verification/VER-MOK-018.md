...
| 2026-08-24 | **One case added and one check's wording corrected, both after the row above and both out of the defect continuous integration found in `WO-MOK-025`'s own candidate. Unlike that row, this one does change coverage, and says so rather than claiming it does not.** **`L34` — *a transcript the platform refuses* — is added under `REQ-MOK-077`**, requiring both hosts to exit `1` with empty standard output and the host's own message prefix when the transcript cannot be read, and requiring the case to be exercised on more than one platform. It is added because this contract had no case for it: **`L32`** covers the parser's exit `2`, **`L8`** covers a mismatch detected while replaying, and `SPEC-MOK-007` rule 13.2's host behaviour on a read that fails had no case of its own on either host — so the required list, run in full, would not have found a defect that let a directory begin a run. `fs::File::open` on a directory succeeds on Linux and refuses only at the first read; both hosts opened the transcript and lent the reader without reading it, so on Linux the engine printed a whole tick before failing and the observer entered the terminal with nothing to end it. It is placed under `REQ-MOK-077` rather than `REQ-MOK-067` because it is a host refusal in the shape of **`L33`**, not a content comparison in the shape of **`L8`**. The case passes on evidence that already exists at `dbc9e6d`, measured on Windows and on Linux, so no requirement's coverage is asserted ahead of its evidence. **`C6` — the credential attestation — withdraws the clause "no check can see this"**, which is stronger than the truth: secret names are enumerable through the hosting platform's API at every scope, measured **0** at repository Actions secrets, Dependabot secrets, Actions variables and each of the two environments, with no organization scope because the account is personal. A value is what no check can see, and a moment other than the one measured is what no measurement covers, so the bullet now says the measurement corroborates the attestation rather than replaces it. **The attestation remains the repository owner's and is unchanged in force.** The restatement in *What a green build does and does not establish* already carried the qualifier "inside the repository" and is correct as written, so it is left alone. | **Approved 2026-08-24 by the repository owner acting as accountable assurance owner**, in a second pass later the same day than the row above, over commit `4cfb297`. Both were raised by the implementation agent of `WO-MOK-025` as escalations **E19** and **E20**: `E19` after the defect was fixed, on finding that running this contract's required list in full would not have caught it, and `E20` on measuring a surface this contract said could not be seen. Neither was repaired when found — `WO-MOK-025` stop-and-escalate condition 6 forbids amending an approved artifact on an implementation agent's judgement, and adding a case to a required list is a widening of it rather than a correction. The alternative for `E19`, recording the gap and deferring it to `WO-MOK-026`, was put with this one and **declined**. `WO-MOK-025`'s *Required verification* enumeration gains **L34** in the same commit, so the list and this matrix agree. `VREC-MOK-024` is prepared after this row and cites the amended text. The implementation agent measured every figure here and wrote the text; it decided neither ruling. |
...
| 2026-08-29 | **One case's floating-point clause is given the reading it already carried, because the clause as written is unsatisfiable jointly with two approved rules. No case is added, no case is removed, no pass condition is weakened, and no requirement's coverage changes.** **L17** — *the transcript's constraints* — keeps all three of its surviving clauses and gains a statement of what the first one means: a floating-point **value**, not any fraction-shaped substring. `WO-MOK-025`'s retained instrument reads the clause the second way and on that reading returns FAIL on **both** of this repository's live transcripts — **503** hits over the accepted run's 515 records and **567** over attempt 1's 579 — while the number of **genuine JSON floating-point values in either file is 0**. Every hit is the same single distinct string, `5.6`, in the same single field, in the same context: the model identifier `gpt-5.6-luna`, once per exchange, stored as the opaque string rule 15.2 calls for. The same instrument returns **0 hits over the 233 records** of the synthetic fixture, which is why the wording survived until a provider was actually called. **The unqualified reading is not merely wide, it is unsatisfiable.** Rule 10.4a, amended 2026-08-29 under `WO-MOK-030`, makes the connector's response carry the model identifier, and rule 15.2 obliges the run record to state it — a decision taken so that a published figure names the model that **answered** rather than the model that was asked for. A conforming live transcript against this provider must therefore carry a dot-decimal substring, and a transcript that satisfied the unqualified clause could not name the model that produced it. **This is the same shape as the closed-alphabet clause the row above withdrew**, where a transcript satisfying the withdrawn clause could not have carried a request. **The clause's substance is untouched.** What it exists for is that no number crosses a boundary as a float — case `P6`, and the reason rule 14.4's cache ratio crosses as basis points — and that property holds in both transcripts exactly. The precedent for stating a reading rather than moving a bound is **L5** in the row above, which "now says which reading it means" while its substance did not change. **What this row does not do**: no case other than `L17` is amended, `L17`'s other three clauses stand word for word, no rule of any specification is amended — rule 11.4 adopts the same constraint for the transcript and is believed correct as written, the word *value* already excluding a substring inside an opaque string — and **`WO-MOK-025`'s instrument is not narrowed**. The consequence of that last one is stated rather than left to be found: the instrument will keep returning FAIL on every live transcript, and after this row that FAIL is a recorded disagreement between a check and the case it implements rather than an open question. The frontmatter's `updated` moves to **2026-08-29**. | **Approved 2026-08-29 by the repository owner acting as accountable assurance owner**, by selecting the presented option, with the alternative measured and displayed: narrowing the instrument to genuine JSON floats was costed and **declined**, the instrument being a file inside `WO-MOK-025`'s evidence packet that `VREC-MOK-024` binds and a change `SPEC-MOK-004` rule 12 makes a strictness change. The finding was raised by the implementation agent of `WO-MOK-026` as an escalation and recorded in that work order's completion report as **L17 PASS IN PART, ESCALATED**, and in `VREC-MOK-025`'s decision section as one of the four findings put to the owner. It was not repaired when found: `WO-MOK-026` stop-and-escalate condition 6 forbids amending an approved artifact on an implementation agent's judgement, and by the time it was decided that work order had reached `implemented` and its record `verified`. **No verification record is re-opened.** `VREC-MOK-025`'s `L17` disposition stands exactly as written: this row states what the contract now requires, not that the candidate's result changed. The implementation agent measured every figure here and wrote the text; it decided none of the substance. |
...
| `REQ-MOK-069` | static check | **L17** the transcript's constraints | no floating-point value, no timestamp and no path appears in a transcript, every text field survives a round trip through the transcript's escaping function unchanged, and two transcripts of the same recorded run compare equal with `cmp`. **The closed-alphabet clause is withdrawn** *(amended 2026-08-24)*: `SPEC-MOK-007` rule 11.4.1 withdrew it the same day, on the measurement that a request's own prose carries spaces, commas, apostrophes, less-than signs and em dashes, none of which the alphabet holds. The round trip is what replaced it and is what this case now checks. **The floating-point clause means a floating-point *value*, and a fraction-shaped substring inside an opaque string is not one** *(amended 2026-08-29)*: `SPEC-MOK-007` rule 10.4a makes the connector's response carry the model identifier and rule 15.2 obliges the run record to state it, so a live transcript against a provider whose identifier carries a dot-decimal version — `gpt-5.6-luna` — must carry a fraction-shaped substring, and a transcript satisfying the unqualified reading could not name the model that answered. The clause is satisfied when no number crosses a boundary as a float, which is case `P6`'s property and why rule 14.4's ratio crosses as basis points; it is **not** a prohibition on the characters of an opaque identifier |
...
| `REQ-MOK-077` | automated test | **L34** a transcript the platform refuses | *(added 2026-08-24)* in both hosts, a transcript path naming something the platform will not read exits `1`, writes nothing to standard output, names the path on standard error behind the host's own prefix, and starts no tick. **The case is exercised on more than one platform, and that is a pass condition rather than a precaution**, because the refusal arrives at a different point on each: `fs::File::open` on a directory succeeds on Linux and fails only at the first read, so a host that opens the transcript and lends the reader without reading it refuses on Windows and *begins the run* on Linux. The message text is the platform's own and is not checked; the exit code, the empty standard output and the prefix are. A host that begins the run fails this case on the standard-output clause, which is exactly where continuous integration found the defect this case exists for |
...
- **C6** The attestation that the credential is not configured in the repository's automation secrets. It is the
  repository owner's statement, retained with the evidence, and it is the single fact the whole cost containment rests
  on. **The clause "no check can see this" is withdrawn** *(amended 2026-08-24)*: secret **names** are enumerable
  through the hosting platform's API at every scope a workflow can read a secret from, and `WO-MOK-025`'s evidence
  retains that measurement — repository Actions secrets, Dependabot secrets, Actions variables and each environment's
  secrets, **0** at every one, with no organization scope to inherit from because the account is a personal one. What no
  check can see is a **value**, and what no measurement establishes is the state of that surface at any moment other
  than the one it was taken at. **So the measurement corroborates the attestation and does not make it**, and a retained
  figure of zero is not a substitute for the owner's statement. Nothing inside the repository sees any of it, which is
  what *What a green build does and does not establish* already says with the qualifier this bullet was missing.
