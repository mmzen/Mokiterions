...
| `REQ-MOK-064` | automated test | **L5** the enumeration is not the core list | at least one request in the transcript enumerates a targeted action, and no request's enumerated set equals the observation's core-proposal list; a run in which no targeted action is ever enumerated fails this case as unexercised |
