...
5. **The governance state of the artifacts this change amends.** The `SPEC-MOK-001` and `SPEC-MOK-003` amendments
   `WO-MOK-011` names must be present and approved before this contract can be satisfied. Their absence fails this
...
| both | static-analysis | **Required amendments present and approved** (oracle 5) | The `SPEC-MOK-001` and `SPEC-MOK-003` amendments `WO-MOK-011` names are approved; and the claim that `SPEC-MOK-002` needs no amendment is checked by interface comparison, not asserted. Absence fails this contract regardless of code state |
...
Retained under `docs/engineering/simulation/evidence/WO-MOK-011/`:
