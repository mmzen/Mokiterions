# WO-MOK-007 amendment approvals — oracle 5

`VER-MOK-007`'s fifth oracle is the governance state of the artifacts this change amends: "an amendment
nobody approved is not a specification", and its absence fails the contract regardless of the state of the
code. This file is generated by `analysis/amendments.py`, which reads the artifacts and the git history.
What it checks, and why each check is not a formality, is in that script's header.

Everything below is measured against the commit this work started from, `60fda9f`, and every row this
work order adds is dated `2026-08-19`.

## 1. The chain's governance state

| Artifact | status | expected | updated | ok |
|---|---|---|---|---|
| `INT-MOK-006` | `approved` | `approved` | 2026-08-19 | yes |
| `CAP-MOK-006` | `approved` | `approved` | 2026-08-19 | yes |
| `REQ-MOK-031` | `approved` | `approved` | 2026-08-19 | yes |
| `REQ-MOK-032` | `approved` | `approved` | 2026-08-19 | yes |
| `REQ-MOK-033` | `approved` | `approved` | 2026-08-19 | yes |
| `REQ-MOK-034` | `approved` | `approved` | 2026-08-19 | yes |
| `VER-MOK-007` | `approved` | `approved` | 2026-08-19 | yes |
| `WO-MOK-007` | `in_progress` | `in_progress` | 2026-08-19 | yes |
| `SPEC-MOK-001` | `approved` | `approved`, amended | 2026-08-19 | yes |
| `SPEC-MOK-002` | `approved` | `approved`, amended | 2026-08-19 | yes |
| `SPEC-MOK-003` | `approved` | `approved`, amended | 2026-08-19 | yes |

The work order is `in_progress` and not `complete`: a work order is closed by a verification record that
binds a commit, and that record is written after the commit it names. `WO-MOK-006` closed the same way.

## 2. The amendment record against the approved list

Each provision `WO-MOK-007` states in full is looked for twice — in the amendment record's 2026-08-19 row,
and in the specification's body — and the two searches are over disjoint text, because a record that
claimed an amendment the text does not carry would otherwise satisfy both with the same sentence.

### `SPEC-MOK-001` — nine provisions amended and one rule appended

- rows dated 2026-08-19: **3**
- approval recorded in the row: yes
- names this work order: yes
- records that the implementation agent wrote the text and did not decide the substance: yes

| Provision | in the record | in the text |
|---|---|---|
| 1. *Scope* drops fear and traits from its exclusions | yes | 2/2 phrases |
| 2. *State model / Mokiterion* gains fear and the trait | yes | 1/1 phrases |
| 3. A *Behavioral trait* subsection | yes | 2/2 phrases |
| 4. *Time and entropy* records the one exception | yes | 1/1 phrases |
| 5. *Inputs* and *Help output* take the third value | yes | 1/1 phrases |
| 6. *Data and interface contracts* carries the trait and fear | yes | 1/1 phrases |
| 7. Rule 1 places the derivation | yes | 1/1 phrases |
| 8. Rule 3 carries the trait | yes | 1/1 phrases |
| 9. Rule 7 traces the pre-update fear, rule 12 updates it | yes | 2/2 phrases |
| addition: rule 19, appended rather than renumbered | yes | 1/1 phrases |
| and rule 5 otherwise untouched | yes | n/a — a statement about the amendment |

### `SPEC-MOK-002` — two entries amended in rule 5, and rule 6 re-checked rather than amended

- rows dated 2026-08-19: **1**
- approval recorded in the row: yes
- names this work order: yes
- records that the implementation agent wrote the text and did not decide the substance: yes

| Provision | in the record | in the text |
|---|---|---|
| 1. `Policy` gains a third variant | yes | 1/1 phrases |
| 2. `AgentSnapshot` carries four `u8` attributes | yes | 1/1 phrases |
| and rule 6 re-checked, not amended | yes | n/a — a statement about the amendment |

### `SPEC-MOK-003` — three provisions amended in rule 4

- rows dated 2026-08-19: **1**
- approval recorded in the row: yes
- names this work order: yes
- records that the implementation agent wrote the text and did not decide the substance: yes

| Provision | in the record | in the text |
|---|---|---|
| 1. The two-line mockup shows four gauges | yes | 1/1 phrases |
| 2. Rule 4's prose reads four attributes, both forms | yes | 2/2 phrases |
| 3. Item 5's reservation becomes a computed value | yes | 2/2 phrases |
| and the reservation's own reasoning retained, not deleted | yes | 1/1 phrases |

Two of those provisions amend by deletion, and no phrase search can show that a phrase is gone. The sentence
that carried it is located instead, and its contents asserted. Each sentence is anchored on a phrase that must
still be in it, at the far end of the list being inspected, so a capture that stopped short of the list fails
rather than reporting an absence it never looked for.

| Artifact | sentence | located, and the anchor in it | no longer names |
|---|---|---|---|
| `SPEC-MOK-001` | *Scope*'s exclusion sentence no longer names fear or traits | yes, 115 characters through `user interface` | yes |
| `SPEC-MOK-003` | rule 10 item 7's list of what the engine does not compute no longer names them | yes, 134 characters through `per-agent entropy` | yes |

Both of those checks, and the provision check above them, were exercised on deliberately broken inputs before
being used, because a check that finds nothing reads the same as a check that looks for nothing — and this one
was wrong in that direction once already. `self_test` in the script injects each failure and asserts that it is
reported. 7 of 7 controls held:

- ok — a provision the record states and the body carries is reported clean
- ok — a provision the record does not state is reported
- ok — a provision whose amended text is absent from the body is reported
- ok — the body and the record are disjoint, so the record's own prose cannot satisfy a body check
- ok — a deletion check whose sentence is not found is reported rather than passed
- ok — a deletion check truncated before its anchor is reported rather than passed
- ok — a forbidden word still present in the sentence is reported

## 3. What was amended beyond the approved list

Three amendments were written during implementation and are not in the list `WO-MOK-007` states. None is left
to be found in a diff: each is written into the specification's own amendment record, and each is named here
with what it still needs. One is approved, because the owner took it as a decision under a stop condition.
The other two are not, and say so.

**`SPEC-MOK-001`.** The trait range narrowed from `0..=100` to `0..=40`, and with it rule 19's upper-bound note and the two acceptance examples that cited unreachable tolerances.

- in the amendment record: yes
- in the specification's body: 2/2 phrases
- **Approved.** The repository owner, acting as technical owner, chose narrowing over amending `REQ-MOK-034`'s survivor floor on 2026-08-19, when `WO-MOK-007` stop condition 6 fired. The work order records the decision and `escalation.md` the measurement it was taken on. The first form of *Behavioral trait* named this amendment as the one to make on exactly this evidence, so it is a foreseen correction rather than an unplanned one.

**`SPEC-MOK-001`.** A correction to this work order's own first amendment: the *Help output* sentence it added required the explanatory prose to state which decision source is the default, contradicting the same section's approved *stated once* paragraph. The default clause is withdrawn; the three-source description stays.

- in the amendment record: yes
- in the specification's body: 2/2 phrases
- **Recorded, not approved — OUTSTANDING.** The contradiction was between two provisions of one section, one approved 2026-08-17 and one 2026-08-19, and the inherited test `cli::each_declared_default_is_stated_once` — bound by a `verified` `VREC-MOK-004` — asserts the side the implementation is already on. Satisfying the withdrawn clause would have meant relaxing that assertion, which `WO-MOK-007` forbids, so the specification is corrected instead. **It is a correction to text the technical owner approved on 2026-08-19 and needs that owner's ratification.**

**`SPEC-MOK-003`.** Three further provisions outside rule 4: the `AgentSnapshot` field list gains `fear`; rule 10 item 7 loses `fear` and traits from its list of values the engine does not compute; rule 11's `decision_source_selected` row gains `REQ-MOK-033` for `individual`.

- in the amendment record: yes
- in the specification's body: 3/3 phrases
- **Recorded, not separately approved.** These were found while implementing and are not in the list `WO-MOK-007` states, so the owner has not approved them as such. Each is forced by the change rather than chosen with it — a field list omitting `fear` would contradict `SPEC-MOK-002` rule 5, an item claiming the engine computes neither `fear` nor traits would be false, and an exhaustive mapping missing a row is a gap the compiler reaches before an operator does — and each is written into the 2026-08-19 amendment row rather than made quietly. **They require the technical owner's ratification, and this artifact is where that obligation is recorded.**

## 4. The earlier layer, left where it was

`WO-MOK-005` left six amendments **OUTSTANDING** across `SPEC-MOK-002`, `SPEC-MOK-003` and `ARCH-MOK-001`,
and the repository owner overrode the gate that would have settled them before this work began. The
mitigation `WO-MOK-007` records is that the two layers remain separable by inspection. That is a checkable
claim, and this is the check: every amendment row dated before this work order's date, compared byte for
byte against `60fda9f`.

| Artifact | rows before this date | identical to the base commit | OUTSTANDING markers, then and now |
|---|---|---|---|
| `SPEC-MOK-001` | 6 | yes | 0, then 0 |
| `SPEC-MOK-002` | 3 | yes | 2, then 2 |
| `SPEC-MOK-003` | 5 | yes | 2, then 2 |

| Artifact | changed since the base commit | why it must not be |
|---|---|---|
| `VREC-MOK-005` | no | the record whose gate was overridden: still `ready`, its six amendments still OUTSTANDING, its seven manual assessments still unrecorded |
| `ARCH-MOK-001` | no | no architecture amendment is required by this work order, and none was made |

`VREC-MOK-005` is `ready`, as it was. This work order does not approve its six
amendments, does not verify it, does not perform its seven manual assessments and does not transition
`WO-MOK-005`. **The override is a cost carried forward, not a debt paid.** So the honest statement of this
oracle is that its second condition — that the amendments already outstanding under `VREC-MOK-005` be
resolved before this change is verified — is **not met**, by the repository owner's explicit decision of
2026-08-19, recorded in `WO-MOK-007` under *The gate was overridden*. What this artifact establishes is the
first condition: the amendments this change itself requires are present, approved, carried by the text, and
separable from the earlier layer.

## Result

**RESULT: PASS** — every artifact in the chain is approved, every provision the owner approved is in both the record and the text, the two provisions that amend by deletion are shown to have deleted, the three amendments beyond the approved list are named with what each needs, and the earlier layer is byte-identical to the commit this work started from. All 7 controls on the checks themselves held, so no result above is a check that looked for nothing. Oracle 5's second condition is unmet by the owner's recorded override, which is stated above rather than counted as a pass.
