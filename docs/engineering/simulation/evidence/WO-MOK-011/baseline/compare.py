"""VER-MOK-011 oracle 1: hash every captured cell, apply the projection, and compare.

Usage, from the repository root:

    python docs/engineering/simulation/evidence/WO-MOK-011/baseline/compare.py \
...
def main():
    ...
    pre = manifest(pre_dir, os.path.join(out_dir, 'pre-manifest.txt'),
                   f'VER-MOK-011 oracle 1: pre-change capture, commit {commit}')
    post = manifest(post_dir, os.path.join(out_dir, 'post-manifest.txt'),
                    'VER-MOK-011 oracle 1: post-change capture, the candidate tree')

    report = [
        'VER-MOK-011 oracle 1 - projected byte comparison',
        '',
