#!/usr/bin/env bash
# Captures VER-MOK-011's declared matrix into $1.
#
# The matrix is the five declared seeds of VER-MOK-002, all three decision sources, the default
# density and the two swept densities, with and without --trace-actions, at 1,000 ticks: 90 runs.
# Exit codes are captured beside each stream, because VER-MOK-011 compares them too.
#
