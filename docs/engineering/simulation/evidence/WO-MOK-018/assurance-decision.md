...
| What lands where — pushing `56d17d9`, `WO-MOK-018`'s status, and PR #31's draft state? | **Push, WO implemented, PR ready** |
...
**The accountable assurance owner accepts the retained evidence for `WO-MOK-018` at commit `50364a3`,
...
- **None of the nine is corrected**, and none is dismissed as not a defect. They stand as
  ...
  `ADR-MOK-005` (2), `SPEC-MOK-002` (1), `WO-MOK-018` (1) and `REQ-MOK-045` (1).
...
| Active work | 1 — `WO-MOK-018` `[in_progress]` | 1, unchanged — **but see below** |
...
**On the *Active work* row.** It stays at 1 through this commit because `WO-MOK-018` is still
...
> `WO-MOK-018`, taking dashboard warnings 7 → 8 and inspector findings 20 → 21 with error counts
> unmoved. The full pair is in `WO-MOK-018`'s *Transition to `implemented`* section, measured in this
...
1. **Oracle 7's script is pinned to the candidate state and must not be re-run as a re-measurement after
   these governance acts.** `analysis/amendments.py` carries `CHAIN` with `WO-MOK-018` at `in_progress`
   ...
   > 1, one finding, *WO-MOK-018 carries status `implemented`, expected `in_progress`*, with two places
...
> commit that followed, which moved `WO-MOK-018` to `implemented`. **The sentence above therefore
