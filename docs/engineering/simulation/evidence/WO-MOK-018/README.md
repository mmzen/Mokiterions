# Evidence: `WO-MOK-018`, the optional structured record stream

This packet is the evidence `VER-MOK-012` requires for `WO-MOK-018`. It is written as the work proceeds
...
**A fourth act followed, by a fourth role.** `WO-MOK-018` moved from `in_progress` to `implemented`, the
...
what they measure rather than for the work order, so the observation now names `WO-MOK-018` alongside
