...
| Dependency surface is confined to the observer | Holds; see rule 3. The figure `SPEC-MOK-003` states as "57 crates" is the `--edges normal` count. With build edges it is 59, without proc-macros 37, and counting both workspace members 61, so the specified number is one of four defensible readings rather than the only one — recorded as disclosure 2 in the completion summary and measured in `dependency-review.txt`, which names all four and the two build-edge-only crates. The three citations in `SPEC-MOK-003` are its lines 59, 584 and 645, re-resolved on 2026-08-19; the 2026-08-18 amendment to rule 5 shifted every line below it, so the 57, 528 and 589 this row named before the amendment no longer resolve. |
