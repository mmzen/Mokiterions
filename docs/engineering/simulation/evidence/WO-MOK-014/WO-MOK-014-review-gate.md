# WO-MOK-014 — the review gate, and the declared requirement set it corrected

**Work order** `WO-MOK-014` · **Branch** `governance/adr-mok-006-third-party-crates` · **Parent commit**
`f40b711b0985316494c6d3ac24f9b0434b0f0ad7` · **Pull request** #33 (draft) · **Date** 2026-08-20

This file records one governed act and the finding that forced it: `WO-MOK-014`'s `implements` relation narrowing from
...
failed, with one diagnostic:
    ...
    Phase: review
    Work order: WO-MOK-014 (implemented)
...
| | |
|---|---|
| `WO-MOK-014.implements` before the correction | `REQ-MOK-026`, `REQ-MOK-036`, `REQ-MOK-050` |
| `WO-MOK-014.verification` | `VER-MOK-014` |
| `VER-MOK-014.verifies` | `REQ-MOK-050` — and nothing else |
...
**Reproduced offline, under the pinned runtime, before anything was edited:**
    ...
    $ <pinned 0.4.0 venv>/python -m se_harness preflight . --work-order WO-MOK-014 --phase review
    Harness preflight: FAIL
    ...
    - [W016] docs/engineering/simulation/work-orders/WO-MOK-014.md: verification coverage is missing REQ-MOK-026,
      REQ-MOK-036
...
`WO-MOK-014-harness.txt` records.
...
| **B** — narrow `implements` to `REQ-MOK-050` | **PASS**, exit 0 | PASS — 107 artifacts, 0/0 | 107 artifacts, **357** relations, 7 warnings | identical |

Neither route adds an error, a warning or a finding, and no snapshot digest is quoted for any of the three scratch
trees: a digest of a tree no commit holds is the defect `VREC-MOK-014` documents.
...
Route A was put to the owner first, with the claim that it would leave `VREC-MOK-014`'s claims untouched. **That claim
...
So route A, followed properly, would have required `VREC-MOK-014` to declare `conforms_to = ["VER-MOK-005",
"VER-MOK-008", "VER-MOK-014"]`. That is not extra assessment work against evidence this packet holds:
...
`implements` moved from `["REQ-MOK-026", "REQ-MOK-036", "REQ-MOK-050"]` to `["REQ-MOK-050"]` on 2026-08-20 by the
...
`REQ-MOK-050`'s, and both amendments stand exactly as approved, each in its own artifact's amendment row and each still
among the twelve amendment preconditions `WO-MOK-014` lists.

| File | Edit |
|---|---|
| `work-orders/WO-MOK-014.md` front matter | `implements` narrowed to `["REQ-MOK-050"]` |
| `work-orders/WO-MOK-014.md` *Lifecycle* | The `architecture` relation's basis re-derived, and a new subsection *The declared requirement set, corrected* |
| `evidence/WO-MOK-014/WO-MOK-014-review-gate.md` | This file |
...
`ARCH-MOK-002` each `addresses` `REQ-MOK-050` directly, and each shares a conforming specification with the work
...
`WO-MOK-014`"* in each requirement's amendment row, which by that same rule is discovery and not coverage.
...
Measured from the repository root after every edit above, including this file:
    ...
    Decision required (1): VREC-MOK-014 [ready] assurance-review
    ...
    $ <pinned 0.4.0 venv>/python -m se_harness preflight . --work-order WO-MOK-014 --phase review
    Harness preflight: PASS | Phase: review | Work order: WO-MOK-014 (implemented) | exit 0

`pytest` is not installed in this environment, so the Python suite runs through `unittest discover`, as
`WO-MOK-014-gates.txt` records. `Cargo.lock` is byte-identical to `origin/master`, and `git status --porcelain` is
...
|---|---|---|
| `WO-MOK-014-harness.txt:114` | The preflight reading manifest, listing `REQ-MOK-026` and `REQ-MOK-036` among the artifacts the chain reaches | Still true of `84b21b9`, the tree it measured. The manifest at this tree holds 19 entries, and the four that left are named above |
| `WO-MOK-014-transition.md:87` and its snapshot table | *"twenty paths where there were nineteen"* | Still true of `d33ecb8`. This file is the twenty-first, and it is named `WO-MOK-014-*` like the other twenty |
| `WO-MOK-014-completion-summary.md:54` | *"Evidence — 19 files"* | Stale by two, and already recorded as stale by one in `WO-MOK-014-transition.md` |
| `VREC-MOK-014`'s `commit`, `artifact_snapshot_sha256`, `verified_at`, `evidence_paths` and its gate figures | Bound to `d33ecb8` | Superseded in the commit that follows this one, on the owner's instruction to re-point the record. The figures for `d33ecb8` remain recorded there as figures of that commit |
...
- **It is not a verification, an assurance decision, a transition of `VREC-MOK-014`, a tag or a release.** It moves one
