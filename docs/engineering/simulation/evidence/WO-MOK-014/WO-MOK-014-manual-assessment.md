# `WO-MOK-014` — the manual assessments of `VER-MOK-014`

| Field | Value |
|---|---|
| Work order | `WO-MOK-014` (crates policy, declared-set comparison) |
| Verification contract | `VER-MOK-014` |
...
`VER-MOK-014:220` states: *"Each of the following is an explicit judgement recorded by the accountable role. An
...
- **Assessment 6.** The owner's act was a choice between four bases `VER-MOK-014:254` enumerates, taken as *accepted on
...
`VER-MOK-014`'s *Manual assessments* list is numbered **1, 2, 3, 4, 6, 7**. There is no assessment 5. `WO-MOK-014`
...
direction that six is the intended set and *"seven"* and *"1 to 5"* were the errors: `WO-MOK-014` is amended to say six,
with a dated note in its *Lifecycle* section recording it. `VER-MOK-014` is not renumbered and the gap stays.
...
|---|---|
| 1 | `VER-MOK-014:104`, `:304`, `:339` |
| 2 | `VER-MOK-014:105`, `:304` |
| 3 | `SPEC-MOK-003:800`; `VER-MOK-014:186`, `:327`, `:336` |
| 4 | `WO-MOK-014:329` |
| 6 | `SPEC-MOK-003:59`, `:794`, `:818`; `SPEC-MOK-005:23`, `:24`; `scripts/check_declared_dependencies.py:1019`, `:1024`; `VER-MOK-014:96`, `:139`, `:190`, `:331` |
...
One reading `WO-MOK-014` left open is now answered by the same amendment. Its exclusions bullet said **"The two manual
...
- `ARCH-MOK-001.md:139`, the new conformance check: *"Review each crate in the engine package's declared set against
  ...
  validates an action."*
- `VER-MOK-014.md:227`, approved as assurance owner: *"With one entry declared, the assessment is about `ratatui`: a
...
- the engine package's declared set is **empty**, and its resolved graph is **one crate — itself — on all three
  release targets** (`WO-MOK-014-graphs.txt`, `WO-MOK-014-counts.txt`). There is no engine entry to judge;
- the observer package's declared set is **one entry, `ratatui 0.30.2`**, `default-features = false`, features
  `crossterm`, `layout-cache`, `underline-color` (`WO-MOK-014-features.txt`). It is the only crate the judgement
...
- no simulation code is in the observer at all — `ARCH-MOK-002`'s dependency direction is one-way and the engine does
  not depend on `mokiterions-tui` on any target (`WO-MOK-014-graphs.txt`).
...
`VER-MOK-014:228` is what asked for it — *"the assessment is recorded even so, because an assessment that is only made
...
`VER-MOK-014:235` asked for exactly this: *"For `ratatui` the record is `ADR-MOK-003`, which decided it before these
...
So this assessment was outstanding **beyond what `WO-MOK-014` anticipated**, and it is now recorded. It was the
...
|---|---|---|
| Declared version, features | `ratatui 0.30.2`, `default-features = false`, `crossterm` + `layout-cache` + `underline-color` | `WO-MOK-014-features.txt` |
| External crates in the observer's graph | 57 Windows · 63 Linux · 62 macOS, 66 in union, counting crate identity as (name, version) | `WO-MOK-014-counts.txt` |
| Crates executing a `build.rs` | 7 Windows · 9 Linux · 9 macOS, 10 in union | `WO-MOK-014-build-scripts.txt` |
| Transitive prohibited-class names | `mio 1.2.2` (`net` on), `signal-hook-mio 0.2.5`, on Linux and macOS | `WO-MOK-014-scan.txt` |
| Advisory and licence status | **not measured, expressly out of scope** by `REQ-MOK-050` | — |
...
`VER-MOK-014:240` asked for it: the owner records that the term list *"covers the capability classes decision 4
...
`VER-MOK-011`'s outstanding fifth, and it is the second assessment outstanding beyond what `WO-MOK-014` anticipated.

What is measured, from `WO-MOK-014-scan.txt`, whose terms are printed by the program itself so the evidence cannot
...
- **What the scan cannot see, stated beside the passing result** (`VER-MOK-014:185`): *"a crate can open a socket
...
failing control in `WO-MOK-014-injection.txt`; and rule 8.4d at `SPEC-MOK-005`, whose disclosure obligation is what
...
- `VER-MOK-014:49` (oracle 3) states the substitution, and `:59` states *"`VREC-MOK-002` is **not edited**, and neither
  is the evidence it binds"*;
- `VER-MOK-014:103` requires the retained evidence to state *"the two approved amendments that moved the stream"*, name
  the binding record, and show the inequality;
- `WO-MOK-014`'s *Required verification* paragraph states the same in full and names this assessment as the assurance
  owner's, at `:303`.

What is measured (`WO-MOK-014-determinism.txt`, `WO-MOK-014-determinism-manifest.txt`), seed `123`, 1,000 ticks, trace
off:
...
**The precision correction, and what happened to it.** `ADR-MOK-006` and `VER-MOK-014` spoke of *"`VREC-MOK-002`'s four
...
artifacts**: five statements in `VER-MOK-014` and one in `ADR-MOK-006` now read *"retained under `VREC-MOK-002` in
...
statement of current state that said otherwise, and `VER-MOK-014` records it in its matrix at `:96`, in its
...
What was measured before the judgement (`WO-MOK-014-scan.txt`, `WO-MOK-014-graphs.txt`):
...
`VER-MOK-014:254` enumerated four bases the owner could record. Three were taken together and the fourth — *"that it is
...
socket types into the Linux and macOS observer, exactly as before, and `VER-MOK-014:331` says so in its residual
...
**Not recorded, and its trigger has not occurred.** `VER-MOK-014:265` conditions it: the owner records, *"at the first
...
**The tension, for the owner.** `VER-MOK-014:220` says without qualification that an unrecorded assessment is
...
or a feature (`WO-MOK-014-workflows.txt`); the check refuses in ten distinct ways when a declaration is edited
(`WO-MOK-014-injection.txt`).
...
| 5 | — | — | Does not exist in `VER-MOK-014` |
...
**No manual assessment of `VER-MOK-014` is outstanding.** Five are recorded and the sixth has not been triggered.
...
Two of the five were outstanding beyond what `WO-MOK-014` anticipated. The work order named only assessment 6 and said
...
approval that would otherwise have discharged its assessment. Both are now recorded, and `WO-MOK-014`'s bullet is
