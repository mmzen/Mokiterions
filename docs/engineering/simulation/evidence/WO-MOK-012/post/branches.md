...
`VER-MOK-012` was written against `REQ-MOK-048`'s **five** branches. The amendment of 2026-08-20 hoisted
...
`REQ-MOK-046`'s constraint; here it is only the count.
...
asks `VER-MOK-012` to measure did not materialize here. `runs.md` §5 records that with the reason it
...
Two boundaries `VER-MOK-012` names explicitly are **not reached** in whole runs, and both are covered by
...
is the intended economy is a product question; the measurement is here because `VER-MOK-012` asks for it
