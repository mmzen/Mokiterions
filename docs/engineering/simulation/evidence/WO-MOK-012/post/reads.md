# `REQ-MOK-050`: what every rule, source and validation path reads

| Field | Value |
|---|---|
| Requirement | `REQ-MOK-050`, *Read no population aggregate anywhere a decision or a rule is made* |
...
`REQ-MOK-050` is discharged "by a documented static examination of every rule evaluation, every
...
They can be read as the temptation `REQ-MOK-050`'s rationale names — "target selection would like a
...
`REQ-MOK-050` requires that "the observation carries no aggregate". The struct is 13 fields, at line
...
about anyone else at all. `REQ-MOK-050` names this case as permitted and bounded.

## 5. The seven aggregate reads, and why each is outside the obligation

`REQ-MOK-050`'s trigger is "the evaluation of any `SPEC-MOK-001` rule that governs a Mokiterion's
...
`REQ-MOK-050` does not itself demand these two lists; `VER-MOK-012`'s retention does, and they belong
...
Mokiterion's own perception — the case `REQ-MOK-050`'s *acceptance examples* single out as satisfying the
...
**`REQ-MOK-050` is met at this candidate.** Every read in every decision source, every behavioral rule
...
The shape `REQ-MOK-050`'s rationale warns about is worth naming against the measurement it would
corrupt: `REQ-MOK-049` is **failing** at this candidate, and the cheapest way to make it pass would be
...
about what is read. And it is not a verification verdict: `VER-MOK-012` is the contract and
