"""VER-MOK-012 oracle 1: digest every captured cell. No projection.

Usage, from the repository root:

    python docs/engineering/simulation/evidence/WO-MOK-012/baseline/manifest.py \
...
`VER-MOK-012` oracle 1 compares the 30 `baseline` cells byte for byte with nothing removed. If a
projection turns out to be needed for `baseline`, the change has touched something `CAP-MOK-009`
...
`REQ-MOK-051`. Their digests are recorded for the same reason the `baseline` ones are: a difference
