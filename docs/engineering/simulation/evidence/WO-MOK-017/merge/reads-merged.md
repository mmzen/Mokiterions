...
`Simulation::food_counts(territory) -> [usize; 3]`, lines 3179-3189, is the only place the
counts are formed. Its callers ahead of the test module:

| caller | line | what it feeds | on a decision path |
|---|---|---|---|
| `Simulation::territory_snapshot` | 2225 | the TUI's `TerritorySnapshot`, an observer surface | no |
| `Simulation::summary` | 3174 | rule 18's `RunSummary`, emitted after the run ends | no |
| `Simulation::summary` | 3175 | rule 18's `RunSummary`, emitted after the run ends | no |
| `Simulation::write_metrics_territory` | 3289 | **UNDESCRIBED -- classify this caller** | no |

That is 4 call sites in 3 functions -- `Simulation::summary` calls it
...
Declared at line 947. A source receives an `&Observation` and an
...
|---|---|---|---|
| `baseline` | 965 | 13 | none |
| `reference` | 987 | 23 | none |
| `individual` | 1128 | 27 | none |
| `social` | 1165 | 28 | none |
