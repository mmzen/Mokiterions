...
# `WO-MOK-019` captured the record stream on this branch at a tree with no `social` policy, so its
...
# `WO-MOK-019/capture.sh`'s form, so `analysis/digest.py` reads either capture with no argument of its
# own and the cells sort into the ninety.
#
# Usage, from the workspace root:
#
#     docs/engineering/simulation/evidence/WO-MOK-019/merge/capture-social.sh <target-dir> [sink]
#
