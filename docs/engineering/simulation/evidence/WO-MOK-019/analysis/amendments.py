...
Usage, from the repository root:
    ...
    python docs/engineering/simulation/evidence/WO-MOK-019/analysis/amendments.py \
...
# Every artifact WO-MOK-019 names, with the status it must carry. A work order in flight is
...
CHAIN = [
    ...
    ('WO-MOK-019', 'work-orders', 'in_progress'),
...
BEYOND = [
    ('SPEC-MOK-004', 'specifications/SPEC-MOK-004.md',
     ...
     ['Recorded test-count figures corrected for `WO-MOK-019`',
...
def self_test():
    ...
    good = ('| 2026-08-20 | states the provision, WO-MOK-019 | Approved |\n'
    ...
    silent = '| 2026-08-20 | says nothing, WO-MOK-019 | Approved |\nthe body carries the provision\n'
    ...
    absent = '| 2026-08-20 | states the provision, WO-MOK-019 | Approved |\nan unrelated body\n'
    ...
    only_row = '| 2026-08-20 | states the provision and body carries, WO-MOK-019 | Approved |\n'
...
def main():
    ...
    add('# WO-MOK-019 amendment approvals — oracle 7')
    add('')
    ...
    for name, path in AMENDED.items():
        ...
        rows = rows_dated(text, DATE)
        mine = [row for row in rows if 'WO-MOK-019' in row]
        add('### `%s`' % name)
        add('')
        ...
        add('|---|---|---|')
        for result in check_provisions(text, PROVISIONS[name], 'WO-MOK-019'):
    ...
    for name, path in AMENDED.items():
        ...
        text = read(os.path.join(DOCS, path))
        mine = [r for r in rows_dated(text, DATE) if 'WO-MOK-019' in r]
...
def report():
    ...
    add('')
    add('`ADR-MOK-005` names three documents to amend and `WO-MOK-019` makes all three approval')
    ...
    for name, path, obliged_by, in_record, in_body in BEYOND:
        text = read(os.path.join(DOCS, path))
        rows = [r for r in rows_dated(text, DATE) if 'WO-MOK-019' in r]
        record = '\n'.join(rows)
        body = body_without_record(text)
