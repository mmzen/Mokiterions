# `WO-MOK-019` completion summary — the optional structured record stream

`WO-MOK-019`'s *Completion report format* fixes sixteen items and their order. This file follows that
order exactly, so a numbered heading below is that clause and not a topic chosen here.

| | |
|---|---|
| Work order | `WO-MOK-019`, Phase 4a, status `in_progress` |
...
kinds and finds none of the four counterexample-only names emitted. `WO-MOK-019` named an outcome label
...
```
python docs/engineering/simulation/evidence/WO-MOK-019/analysis/validate.py <capture-dir> <output-file>
```
...
```
python docs/engineering/simulation/evidence/WO-MOK-019/analysis/replay.py <capture-dir> <output-file>
```
...
`WO-MOK-019`'s *Obligations worth restating* names this the second of three: "Oracles 4, 5 and 6 must
...
- a capture of the **current** shape is compared with nothing removed from either side — this is the
  comparison that says `WO-MOK-019` did not disturb the text stream, and it is what `REQ-MOK-045` is
  really about;
...
which `WO-MOK-019`'s *What may change* admits by name. No other pre-existing test's body or assertions
...
### Defect: the `execute` call sites named in `SPEC-MOK-002` and `WO-MOK-019` do not all exist
...
`mokiterions-tui` as "passing `None`". `WO-MOK-019`'s stop-and-escalate condition 4 anticipates the
same sites.
...
| 4 | `WO-MOK-019` lines 12, 238, 376 | "six cumulative counters" | `SPEC-MOK-006`'s *State model* lists **seven** `u64` counters — 1 crossing + 3 consumption + 1 regeneration + 2 skip reasons — carried in **four** struct fields (`simulation.rs:1441–1447`), plus the per-Mokiterion `Option<u64>` death tick, which is per-agent state and not a run counter. Neither four nor seven is six |
...
`WO-MOK-019`'s "six" matches neither. Defects 8 and 9 do change what was measured, and items 10 and
...
record is written after the commit it names — which is why `WO-MOK-019` is `in_progress` and not
...
>
> **`WO-MOK-019` is `implemented` as of the commit after that one**, on the owner's instruction as
