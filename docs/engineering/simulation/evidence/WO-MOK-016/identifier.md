...
| Field | Value |
|---|---|
| Deliverable | `WO-MOK-016` *Evidence to record* — "`identifier.md`, the per-identifier series, the rank correlations, the band evaluation, and the identifier-exchange comparison" |
| Oracle | `VER-MOK-016` oracle 5, both halves, as amended on 2026-08-20 |
...
The decision that shaped this row is `evidence/WO-MOK-016/escalation.md` §11, and the amendment it produced is in
`VER-MOK-016`'s amendment record. **This file is the measurement, not the decision.**
...
|---|---|---|---|
| Declared | `0, 1, 42, 123, 777` | Part one, the gross-advantage tripwire. Also `REQ-MOK-058`'s floor and lethality bound, `REQ-MOK-014`'s and `REQ-MOK-034`'s floors | Comparability. Every survivor figure in this chain is quoted at these five seeds |
...
twelve and `REQ-MOK-058` binds this source at five, so three of twelve is the survivor cost combat was accepted to
...
The distinction is the whole of `WO-MOK-016` stop condition 6. The exchange test passing and the outcome series
...
- It does not establish that `1.25` is the right bound, only where it came from and that it holds at `1.082` with the
  set it is declared on. Its adequacy is `VER-MOK-016` manual assessment 11, and the assurance owner recorded it.
...
- **`INT-MOK-010`'s risk is stated in the opposite direction to the measurement.** Its text anticipates that the
  ...
  decision, which `WO-MOK-016` stop condition 11 reserves. It is recorded and unmade.
