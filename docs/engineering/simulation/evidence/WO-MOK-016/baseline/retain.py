"""VER-MOK-016 oracle 1: retain the part of a 110 MB capture that a reviewer needs.

Usage, from the repository root:

    python docs/engineering/simulation/evidence/WO-MOK-016/baseline/retain.py \
...
`manifest.py` writes the digests. This script writes the stated subset, and the subset is stated here:
  ...
  * `<out-dir>/summary.txt` -- rule 18's final summary line of every cell, one line per cell. This is
    the artifact `VER-MOK-016` requires "at both commits", and at 90 lines it is the whole of it.
  ...
  * `<out-dir>/full/seed42-baseline-d0.75-traceon.txt` -- one cell whole, 414 KB. It is a *traced*
    ...
    leaked into the source `CAP-MOK-010` forbids it to touch. Oracle 1's load-bearing claim is byte
...
What is deliberately **not** retained whole, with the reason:
  ...
  * A whole traced `reference` or `individual` stream, 3 MB each. Their divergence after the change is
    expected under `REQ-MOK-060` and is *characterized by computed output* over both captures, not by
...
# The seven verbs REQ-MOK-051 adds, and the two fields REQ-MOK-052 adds. Counted by name rather than
...
def main():
    ...
    censuses = [f'# {label}',
                ...
                '# REQ-MOK-051 added no verb and REQ-MOK-052 no field at this commit, so every',
