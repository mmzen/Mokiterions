...
| Matrix row | `VER-MOK-016`, both packages, automated-test, "New event types are authorized" |
...
| Authority | `SPEC-MOK-003` rule 11, as amended 2026-08-20 — three rows added, `REQ-MOK-052` takes none — and `SPEC-MOK-001`'s *Data and interface contracts*, where the vocabulary gains three types |
...
| 4 | `decision_source_selected` | `DecisionSourceSelected` | nested on the source | `REQ-MOK-008` / `REQ-MOK-015` / `REQ-MOK-033` / **`REQ-MOK-057`** |
...
| 11 | **`attack_resolved`** | **`AttackResolved`** | **own arm, added** | **`REQ-MOK-053`** |
| 12 | **`threat_resolved`** | **`ThreatResolved`** | **own arm, added** | **`REQ-MOK-055`** |
| 13 | **`surrender_resolved`** | **`SurrenderResolved`** | **own arm, added** | **`REQ-MOK-056`** |
...
### `REQ-MOK-052` takes no row, and the absence is required

Rule 11's amendment says why: the table maps event types, and `REQ-MOK-052` "authorizes seven verbs while
...
`        (EventType::ThreatResolved, "REQ-MOK-055"),` from `expected` in `tests/authority.rs` and run
...
`EventType::ThreatResolved => "REQ-MOK-056",`, which is `surrender_resolved`'s identifier and therefore
well-formed, existing and wrong:
    ...
    assertion `left == right` failed: threat_resolved
      left: Some("REQ-MOK-056")
     right: Some("REQ-MOK-055")
    test result: FAILED. 3 passed; 1 failed
...
**Control E — the overlay's source row stops naming `social`.** Remove `/ REQ-MOK-057 social` from
...
they are stated as such. The frame bound itself is a different clause of `VER-MOK-016` — "the observer's
