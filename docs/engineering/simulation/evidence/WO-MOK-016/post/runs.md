...
| Invocation | `python docs/engineering/simulation/evidence/WO-MOK-016/analysis/runs.py <capture-dir>` |
...
60 Mokiterions die and 54 of the 59 die of attrition, so extinction there is starvation and not combat. `REQ-MOK-058`'s floor of
...
- **`wait` is `0/0` in every cell.** `REQ-MOK-057`'s branches 2, 3 and 6 delegate to rule 19, which
...
"`VER-MOK-016` measures it rather than assuming it stays rare". This is that measurement.
...
- Nothing here measures `REQ-MOK-060`, which is unimplemented under the owner's approved deferral. The
