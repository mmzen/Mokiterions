...
| Oracle | `VER-MOK-016` *Performance and resilience checks*: "A 10,000-tick run under the `social` source completes without panic and without unbounded growth in retained state, and its composition and survivor figures are captured as evidence rather than bound as obligations." |
...
| Raw output | `post/horizon.txt`, 112 lines, and `post/horizon-memory.txt`, 150 lines, both retained whole; every figure below is transcribed from them |
...
**The figure to put in front of the product owner.** `REQ-MOK-058`'s bound is *at least five of twelve
...
than bound as obligations", and `REQ-MOK-058`'s obligation is stated at tick 1,000 and holds there. The
...
at 1,000 ticks, from the stream and without reading the implementation. `REQ-MOK-058`'s ratification
...
This is what ties every figure above to the released evidence and to `REQ-MOK-058`'s ratified numbers.
...
- **The suffered-attack record's bound is checked and never approached.** 27 of the 74,370 traced lines
  carry the field and none is wider than one entry, against a bound of eleven. `VER-MOK-016` says that
...
- **The survivor figures are evidence and carry no obligation** — the contract says so in as many
  words. §4's comparison against `REQ-MOK-058`'s ratified floor is put to the product owner as a
