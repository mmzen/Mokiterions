...
| Implements | `REQ-MOK-051` … `REQ-MOK-059`. `REQ-MOK-060` was **descoped to `WO-MOK-017`** by the scope amendment of 2026-08-21 (`WO-MOK-016.md` §"Scope amendment of 2026-08-21"), and is measured here anyway — see `post/composition.md` |
...
| Packet size | 157 files, 3,116,832 bytes |
...
- **`REQ-MOK-060` is not implemented**, under the product owner's approved deferral of 2026-08-20, and it
  is **no longer in this work order's scope**: the scope amendment of 2026-08-21 descoped it to
  `WO-MOK-017`, so `implemented` here can mean every requirement in scope is implemented rather than
  every requirement but one. It is still why `post/byte-identity.txt` reads `RESULT: MIXED` — the 60-cell
  divergence that oracle 1 asks to be characterized does not exist to be characterized — and the
  measurement the follow-on is to be decided on is taken here anyway, in `post/composition.md`: the
  ceiling is breached at **27 of 30** default-density territory evaluations, so `WO-MOK-017` inherits the
  curve and not just the question. It is recorded rather than worked around.
- **Two of `VER-MOK-016`'s retention items are not yet written**, and the table at the end of this
...
| 8 | `post/updated-tests.md` | completion report item 13's second clause: the nine retained tests whose bodies changed, with assertion counts before and after — 34 into 38 — and each read by hand, because **six of the nine hold the same count** and a count cannot tell a widened sweep from a weakened check. All six turn out to be one edit, a swept list of three policies becoming four, so every baseline assertion is still made over strictly more sources; §3 reads the renumber's further seven, three of which move an identifier a test asserts; and §1.3 records a stale comment the change left behind, stated rather than fixed because fixing it would move the tree every figure here is bound to. Its measurement is `post/updated-tests.txt` |
| 9 | `post/reads.md` | `REQ-MOK-059` discharged by enumeration: every rule, source and validation path, and the seven readers of a set with the reason each is outside the obligation |
| 10 | `post/world-rules-unchanged.txt` | `REQ-MOK-060`'s permitted-and-forbidden constraint measured both ways: 21 named regions byte-identical across the two commits, 3 controls measured as changed, and §4 on why the permitted regions are unchanged too |
| 11 | `post/gates.txt` | the four gate commands with their exit codes, and the `allow`-attribute, manifest, target and test-independence enumerations of the same contract section |
| 12 | `post/interface.txt` | the public surface enumerated at both commits, 186 entries, diffed against the amended rule 5's four growth rows — 15 declared, 15 measured — with rule 6's ten prohibited names checked and one wording defect in the spec recorded |
| 13 | `post/observer.md` | rule 11's fifteen rows against the code item for item, the compiler as what makes the mapping total, the five tests that run, and **five reverted one-line mutations** — three that fail, one of them at compile time, and two that pass and so bound what nothing checks |
| 14 | `post/merge-recheck.txt` | every command-derived figure in items 11 and 13 re-taken at the merge tip, because a figure produced by a command is a statement about a tree and the merge of 2026-08-21 changed the tree. §0 bounds the rest: of the engine's **eight** changed files, seven carry **0 non-comment changed lines** and the eighth carries two, both of them this branch's own renumber reaching a `println!` string — so no capture and no stream-derived record here can have moved, and the whole substantive change is `master`'s observer rewrite. All four gate commands re-run at exit `0` on a character-for-character identical toolchain, and **one packet claim is falsified**: `gates.txt` §5's four-manifest byte-identity, re-established as 0 non-comment changed lines per manifest with `Cargo.lock` byte-unchanged. Also carries the gate the merge added — `ADR-MOK-006`'s `check_declared_dependencies.py` at exit `0`, which measures the engine's empty dependency set as a comparison in both directions rather than as a prohibition — and the three `observer.md` run figures re-taken by re-applying and reverting the same mutations, 250 → **264** and 128 → **142**, both controls still passing. §7 closes with the eight geometry constants measured unchanged despite `layout.rs` collapsing two log heights into one, because the overlay's height is invariant to the log's |
| 15 | `post/resolution-tables.md` | rules 22, 23 and 24 tabulated over their **whole domains** — 10,201 pairs collapsed onto 21 damage values, 101 fear values, 101 satiety values — with the paper column transcribed from the rules by a reader that never opens `simulation.rs`, the 24 retained test rows and 1,742 released resolutions as the engine's two columns, and **eleven controls**: six in the reader and five in the engine |
| 16 | `entropy.txt` | oracle 2, and the `no entropy draw` constraints of `REQ-MOK-052` to `REQ-MOK-056`, on a quantity no released stream reports: the shared stream's **absolute state**, computed from the placement arithmetic because the suite records only a count, then checked against the engine's own output over **12,780 placed cells** in 90 excerpts. All fifteen declared draw counts reproduced without being given them; the state at all **15** encounter seeds behind the suite's 33 call sites, with the 28 no-draw cases as 4 geometries × 7 verbs; and every draw site in the shipped half enumerated — **9** of them, of which none is reachable by any of the nine resolution, validation and application functions, so the constraint is discharged structurally rather than sampled |
| 17 | `post/delegation.md` | oracle 8's three clauses, held to three different depths, and a quantity the release reports nowhere: the shared stream's **whole-run draw total** under all four sources at matched seeds. The precondition is decoded from rule 12's `fear` pair and censused at **83,657 of 118,201 opportunities** with **0** targeted proposals among them; the two sources part **inside tick 1 at all fifteen matched pairs**, exactly where oracle 8 says they will, so the per-observation comparison is carried to the parting with both proposals and no further. The draw total is recovered from printed regeneration and reported at each run's **last** addition, exact for the whole run in **30 of 60** cells — and checked against two counts arrived at without the stream: `baseline`'s known one-draw-per-opportunity count, recovered with no slack in **6 of the 7** cells that carry a pin, and `post/branches.md`'s branch-6 count, inside the recovered interval **15 of 15**. Read it after item 16, whose §7 is what makes the second control valid |
| 18 | `post/composition.md` | the one record here that measures a requirement **nobody claims is met**. `REQ-MOK-060`'s ceiling is breached at **27 of 30** territory evaluations, `high` class over the line in every one, and 14 of the 15 default-density runs carry a breaching territory — reported as a measurement and never as a failed check, because the descope of 2026-08-21 makes it `WO-MOK-017`'s. What the owner's manual assessment 5 gets from it: the ceiling's stated rationale of "`17` points of headroom above the balanced initial third" measures at **17.2 points**, and the drift these runs carry is **31.3**, so the correction has to remove half the drift rather than shave it. Rule 5's recorded "45 of 61" is found in exactly one of the 120 territory evaluations, and the pre-change capture holds that cell with an identical summary. Every summary is reconstructed from `initialized + regenerated − consumed` rather than transcribed |
| 19 | `post/long-horizon.md` | the 10,000-tick clause on all five declared seeds, not one: **two of them go extinct before the limit**, at ticks 7,890 and 9,511, and the five leave 0, 5, 2, 2 and 0 survivors against `REQ-MOK-058`'s ratified floor of five at tick 1,000 — evidence, as the clause says, and put to the product owner. Retained state measured in both halves: three collections reconstructed over the capture's 73,291 ticks, and peak working set for the two no stream shows, flat at a factor of at most `1.0046` across a twenty-fold tick range on three ladders retained whole |
| 20 | `baseline/capture-state.txt` | the baseline capture's provenance, on the same four columns — `RESULT: PASS` |
| 21 | `baseline/pre-manifest.txt` | the 90-cell matrix by per-cell SHA-256, bytes, lines and exit code |
| 22 | `baseline/cross-check.txt` | two free agreements with `WO-MOK-011`'s measurements — 90 of 90 cells, 212 of 212 test names — `RESULT: PASS` |
| 23 | `baseline/census.txt` | the seven targeted verbs and both new `action_trace` fields at **zero occurrences in 110 MB**, which is this change's absence claim measured on the pre-change side |
...
| `composition.md` | rule 18's final summary per seed under each of the four sources at both commits, with the per-territory per-class ratio, and `REQ-MOK-060`'s ceiling evaluated against it: **breached at 27 of 30** default-density territory evaluations, `high` over the line in every one of the 27, and 14 of the 15 runs carrying a breaching territory. Reported as a measurement and **never as a failed check**, because the descope of 2026-08-21 makes the requirement `WO-MOK-017`'s and failing on an approved deferral would report the deferral as a defect. Carries the two figures manual assessment 5 needs — the ceiling's stated `17` points of headroom measured at **17.2**, against a drift of **31.3** — the one evaluation where `high` is not the widest class, rule 5's recorded "45 of 61" found in exactly one of 120 evaluations, and `baseline`'s exclusion measured rather than assumed. **No summary is transcribed**: each of the six counts is reconstructed from `initialized + regenerated − consumed` and reproduced 120 of 120 |
| `composition.txt` | `analysis/composition.py`'s own output over 120 cells, 554 lines, retained whole: eleven sections and `RESULT: PASS`. Every table in `composition.md` is transcribed from it |
...
| `updated-tests.md` | the nine updated test bodies read one at a time with their diffs, the renumber's seven beside them, and stop condition 8 settled clause by clause — which of the five clauses are measured, which are read, and what neither reaches |
| `updated-tests.txt` | `analysis/updated-tests.py`'s own output over five extracted trees, 1,293 lines, retained whole: three measuring runs and five controls, of which **three must fail**. Carries the `git archive` and `sed` commands that built every tree, the disclosure that this shell's `sed` rewrote one file's line endings whole, and control E measuring that no figure depends on them — two digests that are one |
...
| `merge-recheck.txt` | `gates.txt` §§1 to 5 and `observer.md` §§5, 6 and 9 re-run at the merge of 2026-08-21, in `gates.txt`'s own command-record shape and **superseding neither by edit**. §0 measures what the merge moved, per file: 25 non-documentation files, of which the engine's eight carry **no non-comment changed line but a renumbered `println!` string**, so the engine did not move behaviourally between the two commits and every capture, stream-derived record and engine unit test in this packet is untouched. §§1 to 4 re-run `fmt`, a cold-`clean` `clippy` over 59 units, both `tree` invocations and the five-log test table at 264. §5 is **the one falsified claim** and re-establishes it three ways. §6 records `ADR-MOK-006`'s dependency gate, which did not exist when `gates.txt` was written. §7 re-takes the three `observer.md` run figures — control A identical at `left: 14 right: 15`, controls D and E still passing at 264 and 142 — and measures §9's constants unchanged, with the derivation that makes the overlay's height independent of the log's. §8 is what the file does not establish |
...
| `updated-tests.py` | the reader behind `post/updated-tests.txt`. Answers completion report item 13 by **parsing both trees' test bodies rather than reading a test log**, so a test that kept its name and changed what it asserts is visible where a census cannot see it: `#[test]` bodies lifted out brace-balanced, comments stripped with string literals tracked, and assertion macros counted — `unwrap` and `expect` deliberately not among them, with the consequence stated either way. Cross-checks its source-derived parse against the census's log-derived figures **three ways** — the workspace totals, the per-target table and the recorded set of updated names — so a parse that drifted from `cargo test`'s own count would fail rather than be believed. Pairs an absent name with an added one **by body with each side's own name blanked**, which is how a rename is told from a removal, and **exits non-zero if any of its checks fires** |
...
| `composition.py` | the reader behind `post/composition.md`. **Reconstructs rule 18's six food counts rather than reading them**, from `food_initialized`, `food_regenerated` and `food_consumed`, deriving each resource's territory from its printed coordinate the same way the engine's own `territory_of` does — so a summary that disagreed with its own stream would be caught rather than copied. Computes the standing set twice, by arithmetic and by surviving identifier, so the two paths have to agree. Opens no engine source and needs no capture directory for the pre-change side, which comes entirely from `baseline/summary.txt` and `baseline/init/`. **Exits non-zero if any of its 35 checks fires** — and `REQ-MOK-060`'s ceiling is deliberately not one of them |
...
`composition.py` is the third, and it is the only reader here that measures a requirement nobody claims is
met. Two temptations were available and both refused. The first was a temporary build printing the standing
composition directly; the second was a new test asserting the ceiling — which would have moved
`post/test-census-reconciliation.md`'s fixed census figures and made this reader's own packet inconsistent.
So the composition is reconstructed from the three food events every reviewer already has, and the ceiling's
verdict is printed as a verdict rather than raised as a failure. A script that exited non-zero here would be
reporting the owner's approved descope as a defect.
...
| rule 18's final summary per seed under each of the four sources, at both commits | **held** — `post/composition.md`, from `analysis/composition.py` at exit `0` over **120 cells**, every one digest-matched to its manifest row, with the per-territory per-class ratio the clause asks for. No summary is **transcribed**: each of rule 18's six food counts is reconstructed from the stream's own `food_initialized`, `food_regenerated` and `food_consumed` lines — 39,924 placements and 26,136 consumptions — by two independent paths, an arithmetic count and a surviving-identifier set, which have to agree with each other and with the engine's own line. They do, **120 of 120**, and trace parity holds 60 of 60. "Both commits" is held to four sources on the post-change side and three on the pre-change side, because `social` **did not exist** at the baseline commit: that half is read out of `baseline/summary.txt`'s retained bytes with no capture directory at all, and all **45** shared summaries are identical, which is `post/byte-identity.txt`'s finding arriving from the other direction. On that footing `REQ-MOK-060`'s ceiling is evaluated rather than assumed: **breached at 27 of 30** default-density territory evaluations, `high` over the line in every one of the 27, 14 of the 15 runs carrying a breaching territory, the widest class between 37.3% and 81.6%, and the ceiling's stated `17` points of headroom measured at **17.2** against a drift of **31.3** — so a correction has to remove half the drift and not shave it. The breach is reported as a measurement and never as a failed check: the descope of 2026-08-21 makes it `WO-MOK-017`'s, and a reader exiting non-zero here would be reporting the owner's approved deferral as a defect. `baseline`'s exclusion is measured rather than asserted — all fifteen cells go extinct between ticks 119 and 193, and 12 of the 15 compositions never move at all |
...
| `cargo fmt`, `cargo clippy`, `cargo test`, `cargo tree -p Mokiterions` | **held** — `post/gates.txt`, all four at exit `0`, clippy run from a cleaned target directory so that its exit code is a statement about the code and not about the cache. `cargo test` is retained whole at three commits and referenced rather than restated. The same file carries the section's adjacent enumerations: three `allow` attributes, all pre-existing and all in the observer; the four manifests byte-identical to the baseline's; fifteen test targets unchanged; and zero `#[ignore]`, feature gate, environment read, spawned process or path reference in any test. **All four commands were re-run at the merge of 2026-08-21** and are retained in `post/merge-recheck.txt`, on a toolchain identical to `gates.txt`'s character for character, so nothing there is a toolchain difference: all four exit `0` again, the cold `clippy` over 59 units admits `master`'s 349 new non-comment lines of rendering without one added `allow` attribute, and all five independence enumerations still read zero across 682 new lines of `tests/render.rs`. The manifest clause is the **one figure the merge falsifies** — three of the four now differ from the baseline's, 48 insertions — and it is re-established rather than restated: **0 non-comment changed lines in each of the three** and `Cargo.lock` byte-unchanged, which is the stronger form, because a dependency reached by inheritance would move the lockfile. `ADR-MOK-006` accounts for the comments: it withdrew the prohibition `gates.txt` §3 reasons from and replaced it with a declared set compared against the resolved graph in both directions, and §6 of the re-check runs that comparison at exit `0` with the engine measured at 0 external crates on all three declared targets |
...
**Two items remain, both wholly unwritten.** Nothing is held back any more.
