"""VER-MOK-010 oracle 5, governance half: is every amendment this change needs approved?

Usage, from the repository root:

    python docs/engineering/simulation/evidence/WO-MOK-010/analysis/amendments.py \
        > docs/engineering/simulation/evidence/WO-MOK-010/amendment-approvals.md
...
Five things are measured:
  ...
  2. each provision `WO-MOK-010` states, in the record and in the body;
  ...
  5. the artifacts that must not have changed at all -- `SPEC-MOK-002`, whose "needs no amendment"
     claim is `WO-MOK-010`'s, and every commit-bound verification record.
...
CHAIN = [
    ...
    ('VER-MOK-010', 'verification', 'approved'),
    ('WO-MOK-010', 'work-orders', 'in_progress'),
...
# Every provision WO-MOK-010's *Required amendments* section states, with the phrases that must
...
UNCHANGED = [
    ('specifications/SPEC-MOK-002.md',
     "`WO-MOK-010` claims it needs no amendment; oracle 5's interface half is what checks the claim"),
...
def self_test():
    ...
    good = ('| 2026-08-19 | states the provision, WO-MOK-010 | Approved |\n'
    ...
    silent = '| 2026-08-19 | says nothing, WO-MOK-010 | Approved |\nthe body carries the provision\n'
    ...
    absent = '| 2026-08-19 | states the provision, WO-MOK-010 | Approved |\nan unrelated body\n'
    ...
    only_row = '| 2026-08-19 | states the provision and body carries, WO-MOK-010 | Approved |\n'
...
def main():
    ...
    out = []
    add = out.append
    add('# WO-MOK-010 amendment approvals — oracle 5, governance half')
    add('')
    add("`VER-MOK-010`'s fifth oracle is the governance state of the artifacts this change amends:")
    ...
    add('for item at both commits, which is what checks `WO-MOK-010`\'s claim that `SPEC-MOK-002` needs')
    ...
    add('')
    add('Each provision `WO-MOK-010` states in full is looked for twice — in the amendment record’s')
    ...
    for artifact in ('SPEC-MOK-001', 'SPEC-MOK-003'):
        ...
        rows = rows_dated(text, DATE)
        mine = [row for row in rows if 'WO-MOK-010' in row]
        add(f'### `{artifact}`')
        add('')
        ...
        add('|---|---|---|')
        for result in check_provisions(text, PROVISIONS[artifact], 'WO-MOK-010'):
    ...
    add('')
    add("`WO-MOK-010` states amendments to two specifications. A third was amended: `SPEC-MOK-004`,")
    ...
    rows4 = [row for row in rows_dated(spec4, DATE) if 'WO-MOK-010' in row]
    ...
    add('%s and are recorded in `WO-MOK-010`: that the name reaches the observer through the' % DATE)
    ...
    add('`WO-MOK-010` states are present in both the amendment record and the specification text, the')
    ...
    add("interface enumeration in `interface.txt`, and `VER-MOK-010`'s manual assessment 5 is recorded")
