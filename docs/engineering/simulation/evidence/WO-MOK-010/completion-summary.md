...
| Field | Value |
|---|---|
| Work order | `WO-MOK-010` (Phase 2.5, naming) |
...
| Verification contract | `VER-MOK-010` |
...
The sections below follow `WO-MOK-010`'s *Completion report format*, in its order. Every figure is a
...
`REQ-MOK-041`, `VER-MOK-010`, `WO-MOK-010`, and this evidence packet.
...
digest as well, which also measures that the revert was complete. That is `WO-MOK-010`'s
...
Three of the twelve exist because `VER-MOK-010`'s coverage matrix was read back against the
...
`INT-MOK-008`, `CAP-MOK-008`, `REQ-MOK-040`, `REQ-MOK-041`, `VER-MOK-010` and `WO-MOK-010`. The
twelve names and their assignment are the product owner's decision of the same date, recorded in
`WO-MOK-010`; the name's position in the record and the decision that the observer sources it from
...
`WO-MOK-010` and did not decide the substance."
...
recorded in `WO-MOK-010`. The implementation agent wrote the text and did not decide the substance;
...
> **Recorded test-count figures corrected for `WO-MOK-010`, which adds twelve tests for `REQ-MOK-040`
...
**Approval:** "Approved 2026-08-19 by the repository owner acting as technical owner, together with
`INT-MOK-008`, `CAP-MOK-008`, `REQ-MOK-040`, `REQ-MOK-041`, `VER-MOK-010`, `WO-MOK-010` and the
...
decided against. `VER-MOK-010`: "An unrecorded assessment is an outstanding assessment, and this
...
rejecting a naming step that draws. `VER-MOK-010` itself records under *Residual uncertainty* that
neither mitigation is a proof, which is why the assessment exists.

`VER-MOK-010` is therefore **not yet satisfied**, on this one count and no other. Nothing in the code
...
1. **Manual assessment 5 is outstanding** (§10). `VER-MOK-010` is unsatisfied until the assurance
   owner records it.
2. **Two existing inline tests were edited beyond the work order's "and only these" list.**
   `WO-MOK-010` names `tests/spatial.rs`'s glyph case and three glyph call sites as the only tests to
...
4. **`VER-MOK-010`'s wording deviates in one place, and the deviation is in this packet's favour.**
...
7. **Oracle 1's captures are reproducible rather than retained whole.** The 90 raw pre-change streams
   ...
   a failed reproduction. This is the retention form `VER-MOK-010` and `baseline/compare.py` state,
...
**`VREC-MOK-010` is a separate commit-bound record that this work order does not write and cannot
self-approve.** `WO-MOK-010` classifies commit-bound verification as `required`. The verification
