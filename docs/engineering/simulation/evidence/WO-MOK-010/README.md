# `WO-MOK-010` evidence packet — index
...
Start with `completion-summary.md`. It reports every figure in `WO-MOK-010`'s stated order and cites
...
| 2 | `manual-assessment.md` | `VER-MOK-010`'s seven manual assessments; **assessment 5 is OUTSTANDING** |
...
| 10 | `static-checks.txt` | the seven static searches `VER-MOK-010` retains |
...
reasonable thing to do to this repository. What is retained is the form `VER-MOK-010` and
...
`additivity.txt` at the root, because those are the paths `WO-MOK-010`'s evidence list names. The
...
`VREC-MOK-010` is not in this packet. It is a separate commit-bound verification record, written by
the assurance owner against the commit that carries this implementation, and the implementation agent
neither writes nor approves it. `VER-MOK-010` is not satisfied while manual assessment 5 is
