# `WO-MOK-010` — the seven manual assessments of `VER-MOK-010`
...
`VER-MOK-010` states: "Each of the following is an explicit judgement recorded by the accountable
...
the same act as `WO-MOK-010`. **One is outstanding**: assessment 5 concerns the projection, which is
...
`CAP-MOK-008`, `REQ-MOK-040`, `REQ-MOK-041`, `VER-MOK-010` and `WO-MOK-010`.
...
it rests on the owner having chosen the register and approved the table. `VER-MOK-010` records under
...
`VER-MOK-010` states under *Residual uncertainty* that "oracle 1 depends on its projection" and that
...
`VER-MOK-010` is therefore **not yet satisfied**, on one count and one only, and `VREC-MOK-010`
