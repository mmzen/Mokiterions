...
reader who hashes the files below will find one recorded digest that no longer reproduces, and because nineteen
...
**Not rewritten — 19 files, byte-identical.** Every retained `.txt` in this packet:
...
These are program output. Twenty-five lines across them read `WO-MOK-007` or `VER-MOK-007`, all of them in headers the
...
- `observer/roster-frames.txt` was captured against `SPEC-MOK-003` rule 5's tier table, which `WO-MOK-005` withdrew on
  ...
  re-deriving against the merged tree.
