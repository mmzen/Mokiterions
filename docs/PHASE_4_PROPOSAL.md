...
| `WO-MOK-019` | work order | Phase 4a implementation |
...
record no override, and `WO-MOK-019` was written to state the precondition status factually rather than to assert it
...
`WO-MOK-005`. It is not this chain's row and `WO-MOK-019` does not claim it, but this chain proposes to add a row
